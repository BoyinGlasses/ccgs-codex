...
description: "In a CCGS game project, re-evaluate open bugs — priority vs severity, assign to sprints, surface systemic trends. Run when the count grows."
---
...
- After `$ccgs-team-qa` completes and new bugs have been filed
...
**Modes:**
- `$ccgs-bug-triage sprint` — triage against the current sprint; assign fixable bugs
  to the sprint backlog; defer the rest
- `$ccgs-bug-triage full` — full triage of all bugs regardless of sprint scope
- `$ccgs-bug-triage trend` — trend analysis only (no assignment); read-only report
...
- If any S1 bugs are unassigned: "S1 bugs must be assigned before the sprint
  can be considered healthy. Run `$ccgs-sprint-status` to see current capacity."
- If regression bugs exist: "Regressions found — consider re-opening the
  affected stories in sprint tracking and running `$ccgs-smoke-check` to re-gate."
