...
description: "In a CCGS game project, break one epic into implementable stories embedding TR-ID, ADR guidance, acceptance criteria. Reads the control manifest. After $ccgs-create-epics."
---
...
**Previous step:** `$ccgs-create-epics [system]`
**Next step after stories exist:** `$ccgs-story-readiness [story-path]` then `$ccgs-dev-story [story-path]`
...
- `$ccgs-create-stories production/epics/combat/EPIC.md` — full path also accepted
- No argument — at `minimal` there are no epics yet (Option A): skip to Step 2's
  ...
  > "No epics found under `production/epics/`. Run `$ccgs-create-epics layer: foundation`
  > first — an epic is what this skill decomposes."
  >
  > **The zero-epic path is load-bearing.** Asking which epic *and* globbing to
  > list them leaves an `Codex user-input tool` with nothing to offer when the glob is
  > empty. Route to `$ccgs-create-epics` instead — it is named as **Previous step**
...
> `minimal` there is no `$ccgs-create-epics` step and no `EPIC.md`. Instead:
...
>    this is the container `$ccgs-dev-story` and `$ccgs-sprint-status` expect.
...
For `standard`/`full` (a `$ccgs-create-epics` epic exists), read in full (these are small):
...
3. **Capture the `## Last Verified` date**:
   ...
   are absent. This becomes the story's `ADR Version` stamp — `$ccgs-dev-story`
...
content directly and flag the ADR for `$ccgs-architecture-decision [file]
retrofit`.
...
> Check the filename in the epic's Governing ADRs list, or run `$ccgs-architecture-decision`
...
The type determines what test evidence is required before `$ccgs-story-done` can close the story.
...
- **Governing ADR**: which ADR governs how to implement this?
  ...
  - `Status: Proposed` → set story `Status: Blocked` with note: "BLOCKED: ADR-NNNN is Proposed — run `$ccgs-architecture-decision` to advance it"
...
**Prefer an existing QA plan when one already covers a story** — this substitutes for the qa-lead's specs, it does not add a spawn. Glob `production/qa/qa-plan-*.md` for the most recent file; if it holds test specs for stories in this epic (match titles/slugs in its Automated Tests Required section) that differ from the qa-lead's, use `Codex user-input tool` (Use QA-plan specs / Use qa-lead specs / Skip and leave `*Test cases not yet defined — run $ccgs-qa-plan to generate them.*`). Either way no additional `qa-lead` spawn occurs.
...
>   > `$ccgs-dev-story` Phase 3 spawns the engine specialist as a mandatory secondary
...
Use `Codex user-input tool` to close with context-aware next steps:

Check:
- Are there other epics in `production/epics/` without stories yet? List them.
- Is this the last epic? If so, include `$ccgs-sprint-plan` as an option.

Widget:
- Prompt: "[N] stories written to `production/epics/[epic-slug]/`. What next?"
- Options (include all that apply):
  - `[A] Start implementing — run $ccgs-story-readiness [first-story-path]` (Recommended)
  - `[B] Create stories for [next-epic-slug] — run $ccgs-create-stories [slug]` (only if other epics have no stories yet)
  - `[C] Plan the sprint — run $ccgs-sprint-plan new` (only if all epics have stories)
...
- **Verdict: COMPLETE** — [N] stories written to `production/epics/[epic-slug]/`. Run `$ccgs-story-readiness` → `$ccgs-dev-story` to begin implementation.
