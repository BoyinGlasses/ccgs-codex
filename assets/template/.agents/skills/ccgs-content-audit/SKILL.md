...
description: "In a CCGS game project, audit GDD content counts against what's implemented — planned vs built."
---
...
2. **Registry-first count**: If `design/registry/entities.yaml` exists, read it
   ...
   **If `design/registry/entities.yaml` does not exist or has no entries** (it
   ships as an empty stub until `$ccgs-design-system` populates it), skip this step
...
to `$ccgs-create-stories [epic-slug]` or `$ccgs-quick-design` depending on the size of the gap.

### --summary mode

Print the Gap Table and Summary directly to conversation. Do not write a file.
End with: "Run `$ccgs-content-audit` without `--summary` to write the full report."
...
- If any system is `NOT STARTED` and MVP-tagged → "Run `$ccgs-design-system [name]` to
  add missing content counts to the GDD before implementation begins."
- If total gap is >50% → "Run `$ccgs-sprint-plan` to allocate content work across upcoming sprints."
- If backlog stories are needed → "Run `$ccgs-create-stories [epic-slug]` for each HIGH PRIORITY gap."
- If `--summary` was used → "Run `$ccgs-content-audit` (no flag) to write the full report to `docs/`."
