...
description: "In a CCGS game project, map test coverage to GDD critical paths, find fixed bugs lacking regression tests, flag drift from new features."
---
...
- Before a release gate (`$ccgs-gate-check polish` requires regression suite exists)
...
**Modes:**
- `$ccgs-regression-suite update` — scan new bug fixes this sprint and check
  for regression test presence; add new tests to the suite manifest
- `$ccgs-regression-suite audit` — full audit of all GDD critical paths vs.
  existing test coverage; flag paths with no regression test
- `$ccgs-regression-suite report` — read-only status report (no writes); suitable
  for sprint reviews
- No argument — if a sprint is clearly active (sprint plan exists with in-progress stories), run `update`. If ambiguous or no active sprint is detected, use `Codex user-input tool`:
...
> side was empty and the skill that produces it (`$ccgs-map-systems` and
> `$ccgs-design-system` for GDDs, `$ccgs-test-setup` for the test scaffold).
>
...
> same defect `$ccgs-scope-check` carries a Phase 4 guard against, in the same words:
...
- For each HIGH priority gap: "Consider creating the missing regression test
  before the next sprint. Run `$ccgs-test-helpers` to scaffold the test file."
...
- If coverage drift detected: "Regression suite may be drifting. Consider
  running `$ccgs-regression-suite audit` at the next sprint boundary."
