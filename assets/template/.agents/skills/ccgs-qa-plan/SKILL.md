---
name: ccgs-qa-plan
description: "In a CCGS game project, qA test plan for a sprint — classifies stories by Logic/Integration/Visual/UI, covers automated tests, manual cases, smoke scope."
---
...
At runtime, execute this shell snippet if needed: `source "${CLAUDE_PROJECT_DIR:-.}/.claude/hooks/yaml-helper.sh" 2>/dev/null && resolve_config --keys automation,workflow,qa.level,system_overrides`
...
**Argument:** `$ARGUMENTS` (blank = ask user via Codex user-input tool)
...
- **No argument** — use `Codex user-input tool`:
...
> - no file in `production/sprints/` → "No sprint plan found. Run `$ccgs-sprint-plan new`."
> - a sprint plan exists but references no stories → "Sprint plan `[path]` lists no
>   stories. Run `$ccgs-create-stories [epic-slug]`."
...
> all, had nothing. The same shape appears in `$ccgs-story-readiness`, where `NOT ASSESSED`
...
*Smoke tests are verified by the developer via `$ccgs-smoke-check`. Reference this
...
- [ ] Smoke check passes (run `$ccgs-smoke-check sprint` before QA hand-off) *(qa.level standard/full)*
- [ ] No regressions introduced
- [ ] Code reviewed (via `$ccgs-code-review` or documented peer review)
- [ ] Story file updated to `Status: Complete` (via `$ccgs-story-done`)
````
...
then ask two questions together using `Codex user-input tool`:
...
options:
  ...
  - "Also back-fill test case specs into each story file's ## QA Test Cases section (Recommended — enables $ccgs-dev-story and $ccgs-code-review traceability)"
```
...
- Once all sprint stories are implemented, run `$ccgs-smoke-check sprint` to gate QA hand-off — not yet, only after implementation is complete
- For Logic/Integration stories, create the test files at the listed paths
  before marking stories done — `$ccgs-story-done` checks for them"
...
- Use `Codex user-input tool` for scope selection when no argument is provided.
