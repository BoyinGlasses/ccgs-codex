---
name: ccgs-test-evidence-review
description: "In a CCGS game project, quality review of test files and evidence — goes beyond existence, evaluates assertion coverage. ADEQUATE/INCOMPLETE/MISSING/NOT ASSESSED per story."
---
...
`$ccgs-smoke-check` verifies that test files **exist** and **pass**. This skill
...
**When to run:**
- Before QA hand-off sign-off (`$ccgs-team-qa` Phase 5)
...
**Modes:**
- `$ccgs-test-evidence-review [story-path]` — review a single story's evidence
- `$ccgs-test-evidence-review sprint` — review all stories in the current sprint
- `$ccgs-test-evidence-review [system-name]` — review all stories in an epic/system
...
> path was empty, and route: no sprint file → `$ccgs-sprint-plan new`; a sprint plan
> listing no stories → `$ccgs-create-stories [epic-slug]`; a `[system-name]` glob that
...
- For BLOCKING items: "These must be resolved before `$ccgs-story-done` can mark the
  story Complete. Would you like to address any of them now?"
- For thin assertions: "Consider running `$ccgs-test-helpers [system]` to see
