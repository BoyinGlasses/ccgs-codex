...
description: "In a CCGS game project, lightweight spec for small changes — tuning adjustments, minor mechanics. Embeds directly into stories; skips full GDD."
---
...
Full GDD authoring via `$ccgs-design-system` is the heavyweight path. Use this skill
...
**When to run:** Anytime a change is too small for `$ccgs-design-system` but too
...
- Options:
  ...
  - `[F] This is too large — redirect me to $ccgs-design-system`

If [F]: stop. Verdict: **REDIRECTED** — use `$ccgs-design-system` for this change.
...
Present the draft to the user in full. Then use `Codex user-input tool`:
...
- Options:
  ...
  - `[C] This grew too large — redirect to $ccgs-design-system instead`

If [B]: collect the requested changes, revise the draft, and re-present this widget.
If [C]: stop. Verdict: **REDIRECTED** — use `$ccgs-design-system` for this change.
...
Next step: This spec is ready for `$ccgs-story-readiness` validation before
...
Quick Design Specs **bypass** `$ccgs-design-review` and `$ccgs-review-all-gdds` by
...
using `$ccgs-design-system` to author a full GDD for this."

---

## Recommended Next Steps

- Run `$ccgs-story-readiness [story-path]` to validate the story before implementation begins — reference this spec in the story's GDD Reference field
- Run `$ccgs-dev-story [story-path]` to implement once the story passes readiness checks
- If the change is larger than expected, run `$ccgs-design-system [system-name]` to author a full GDD instead
