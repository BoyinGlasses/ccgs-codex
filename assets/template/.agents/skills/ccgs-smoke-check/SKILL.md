...
description: "In a CCGS game project, critical-path smoke gate before QA hand-off — runs the automated suite. A failed check means the build is not QA-ready."
---
...
At runtime, execute this shell snippet if needed: `source "${CLAUDE_PROJECT_DIR:-.}/.claude/hooks/yaml-helper.sh" 2>/dev/null && resolve_config --keys automation,qa.level,testing.strict`
# Smoke Check
...
Arguments can be combined: `$ccgs-smoke-check sprint --platform console`
...
1. **Test framework check**: verify that **game** test files exist — not merely
   ...
   `tests/unit|integration|smoke`. Run `$ccgs-test-setup` to scaffold the testing
   ...
   > **Do not gate on `tests/` existing.** `$ccgs-test-setup` creates `tests/unit/`
...
5. **QA plan check**: glob `production/qa/qa-plan-*.md` and take the most
   ...
   `$ccgs-qa-plan sprint` before smoke-checking for best results."
...
`.claude/docs/technical-preferences.md`. Run `$ccgs-setup-engine` to specify the
engine, then re-run `$ccgs-smoke-check`."
...
3. If the `quick` argument was passed, skip this phase entirely and note:
   "Coverage scan skipped — run `$ccgs-smoke-check sprint` for full coverage
   analysis."
...
appear prominently in the report and must be resolved before `$ccgs-story-done` can
...
project's own smoke definitions were indistinguishable. `$ccgs-gate-check` now
...
Use `Codex user-input tool` to batch-verify. Keep to at most 3 calls.
...
**Engine**: [engine]
**QA Plan**: [path, or "Not found — run $ccgs-qa-plan first"]
...
3. Else default to **blocking** — an unset `testing.strict.config` keeps
   ...
   reciprocal carve-out is recorded in `.agents/skills/ccgs-story-done/SKILL.md` and
...
Fix the failures and run `$ccgs-smoke-check` again to re-gate before QA hand-off."
...
agent to begin manual verification. Re-run `$ccgs-smoke-check` once the failures
are fixed."
...
result from your IDE or CI', 'run `$ccgs-setup-engine`', 'produce a build'.]

Re-run `$ccgs-smoke-check` once any of those is resolved."
...
Advisory items to resolve before running `$ccgs-story-done` on affected stories:
