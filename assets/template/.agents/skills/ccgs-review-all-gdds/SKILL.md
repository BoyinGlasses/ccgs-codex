...
description: "In a CCGS game project, holistic cross-GDD review — contradictions between systems, dominant strategies, economic imbalance, cognitive overload, pillar drift."
---
...
At runtime, execute this shell snippet if needed: `source "${CLAUDE_PROJECT_DIR:-.}/.claude/hooks/yaml-helper.sh" 2>/dev/null && resolve_config --keys automation,workflow,system_overrides`
# Review All GDDs
...
**This is distinct from `$ccgs-design-review`**, which reviews one GDD for internal
...
- Before `$ccgs-create-architecture` begins (architecture built on inconsistent GDDs
...
Run `$ccgs-consistency-check` after this review to populate the registry."
...
checklist item here (`$ccgs-design-review` owns per-GDD completeness). Loading them
...
> then re-run `$ccgs-review-all-gdds`."
...
tells authors it is machine-checked here, and `$ccgs-design-system` requires it
...
Use `Codex user-input tool` for write permission:
...
If any GDDs are flagged for revision, use a second `Codex user-input tool`:
...
append to `production/session-state/active.md`:
    ...
    ## Session Extract — $ccgs-review-all-gdds [date]
    - Verdict: [PASS / NOT ASSESSED / CONCERNS / FAIL]
...
After all file writes are complete, use `Codex user-input tool` for a closing widget.
...
- Are any GDDs in the "Flagged for Revision" table? → offer $ccgs-design-review option for each
- Read systems-index.md for the next system with Status: Not Started → offer $ccgs-design-system option
- Is the verdict PASS or CONCERNS? → offer $ccgs-gate-check or $ccgs-create-architecture
...
- `[_] Run $ccgs-design-review [flagged-gdd-path] — address flagged warnings` (one per flagged GDD, if any)
- `[_] Run $ccgs-design-system [next-system] — next in design order` (always include, name the actual system)
- `[_] Run $ccgs-create-architecture — begin architecture (verdict is PASS/CONCERNS)` (include if verdict is not FAIL)
- `[_] Run $ccgs-gate-check — validate Systems Design phase gate` (include if verdict is PASS)
- `[_] Stop here`
