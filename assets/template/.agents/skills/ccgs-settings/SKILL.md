---
name: ccgs-settings
description: "In a CCGS game project, view or change project config — effective merged values, or set locally in project.local.yaml."
---
...
# $ccgs-settings — view or change project config
...
`automation_always_ask` category), so `$ccgs-settings` confirms config writes even
...
| Form | Mode |
|------|------|
| `$ccgs-settings` (no args) | **View-all** (Phase 2) |
| `$ccgs-settings <key>` (single arg, no `=`) | **View-one** (Phase 3) |
| `$ccgs-settings <key>=<value>` (single arg with `=`) | **Set-yaml** (Phase 4) |
| `$ccgs-settings --local <key>=<value>` (`--local` prefix) | **Set-local** (Phase 5) |
...
first, then check the remaining operand the same way (so `$ccgs-settings --local =foo`
and `$ccgs-settings --local key=` both go to Phase 6).
...
> `$ccgs-settings` register as a reader of two settings nothing reads, and the gate
...
1. Confirm `project.yaml` exists. If missing, report:
   > "No `project.yaml` found — run `$ccgs-start` to create one."
   And exit.
...
$ccgs-settings <key>=<value>, and an explicit value wins over rigor.
```
...
```
  Set explicitly with: $ccgs-settings <key>=<value>   (an explicit value wins over rigor)
```
...
4. **Approval gate**: use `Codex user-input tool`:
...
1. **Whitelist check**: call `is_locally_overridable <key>`.
   - If it returns 1 (locked), print one line:
     > ``<key> cannot be locally overridden — it's a project-wide setting. Use `$ccgs-settings <key>=<value>` (no `--local`) to change it for the whole team.``
...
3. **Base check**: if `project.yaml` doesn't exist:
   > "Cannot create `project.local.yaml` — `project.yaml` is missing. Run `$ccgs-start` to create one first."
...
```
Usage:
  $ccgs-settings                          — view all effective config
  $ccgs-settings <key>                    — view a single setting
  $ccgs-settings <key>=<value>            — change a setting (project.yaml)
  $ccgs-settings --local <key>=<value>    — change a setting locally
                                       (project.local.yaml, gitignored)

Examples:
  $ccgs-settings modes.review_mode
  $ccgs-settings modes.review_mode=full
  $ccgs-settings --local modes.automation=autonomous
```
