---
name: ccgs-consistency-check
description: "In a CCGS game project, scan GDDs against the entity registry for cross-document conflicts. Grep-first approach targets conflicting sections, different stats."
---
...
**This skill is the write-time safety net.** It catches what `$ccgs-design-system`'s
per-section checks may have missed and what `$ccgs-review-all-gdds`'s holistic review
catches too late.

**When to run:**
- After writing each new GDD (before moving to the next system)
- Before `$ccgs-review-all-gdds` (so that skill starts with a clean baseline)
- Before `$ccgs-create-architecture` (inconsistencies poison downstream ADRs)
- On demand: `$ccgs-consistency-check entity:[name]` to check one entity specifically
...
> "Entity registry is empty. Run `$ccgs-design-system` to write GDDs — the registry
...
```markdown
### [YYYY-MM-DD] — $ccgs-consistency-check — 🔴 CONFLICT
...
<!-- Auto-maintained by $ccgs-consistency-check. Do not edit manually. -->
...
Then close with an `Codex user-input tool` widget:
...
- **Options**:
  ...
  - `[C] Run $ccgs-design-review on the most conflicted GDD`
  - `[D] Stop here`
...
- **If PASS**: Run `$ccgs-review-all-gdds` for holistic design-theory review, or
  `$ccgs-create-architecture` if all MVP GDDs are complete.
- **If CONFLICTS FOUND**: Fix the flagged GDDs, then re-run
  `$ccgs-consistency-check` to confirm resolution.
- **If STALE REGISTRY**: Update the registry (Phase 6), then re-run to verify.
- Run `$ccgs-consistency-check` after writing each new GDD to catch issues early,
