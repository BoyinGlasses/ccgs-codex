...
description: "In a CCGS game project, orchestrate the level team — level-designer, narrative-director, world-builder, art-director, systems-designer, qa-tester — for complete area creation."
---
...
**Decision Points:** At each step transition, use `Codex user-input tool` to present
...
**Gate**: Use `Codex user-input tool` to present all three Step 1 outputs (narrative brief, lore foundation, visual direction targets) and confirm before proceeding to Step 2.
...
- (b) Pause and run `$ccgs-team-level [area-name]` first to establish that area

Do NOT invent content for the missing adjacent area.

**Gate**: Use `Codex user-input tool` to present Step 2 layout (including any unresolved adjacent area dependencies) and confirm before proceeding to Step 3.
...
**Gate**: Use `Codex user-input tool` to present Step 3 outputs and confirm before proceeding to Step 4.
...
**Gate**: Use `Codex user-input tool` to present both Step 4 results. If the accessibility-specialist returned any BLOCKING concerns, highlight them prominently and offer:
...
`Codex user-input tool`: "May I write the compiled level design to
...
phase is gated by an `Codex user-input tool`. A sub-agent does **not** prompt per write
...
- Run `$ccgs-design-review design/levels/[level-name].md` to validate the completed level design doc.
- Run `$ccgs-dev-story` to implement level content once the design is approved.
- Run `$ccgs-qa-plan` to generate a QA test plan for this level.
