---
name: ccgs-start
description: "In a CCGS game project, first-time onboarding — asks where you are, then guides you to the right workflow."
---
...
> **`$ccgs-start` never writes `modes.review_mode`, in either location.** It is a
...
`modes.automation` is set yet, so `$ccgs-start` runs collaboratively — it is
...
`$ccgs-setup-engine`, which Phase 4 hands off to.
If `$ccgs-start` is re-run on an already-configured project, the resolved mode
...
This is the first thing the user sees. Use `Codex user-input tool` with these exact options so the user can click rather than type:
...
2. Briefly explain what `$ccgs-brainstorm` does: it turns "no idea" into a written design your next step can build from. Mention that it has two modes: `$ccgs-brainstorm open` for fully open exploration, or `$ccgs-brainstorm [hint]` if they have even a vague theme (e.g., "space", "cozy", "horror").

   > **Describe it tier-neutrally here.** Phase 3d has not run, so you do not yet
   > know how much process this project wants — and `$ccgs-brainstorm` changes shape
   ...
   > contradicting accounts of one skill inside a single `$ccgs-start` run. Phase 4 is
   > where the tier-specific description belongs.
3. Recommend running `$ccgs-brainstorm open` as the next step, but invite them to use a hint if something comes to mind
4. Name the **immediate next step only** — `$ccgs-brainstorm open`. Do not list the
...
3. Recommend running `$ccgs-brainstorm [their hint]` to develop it
4. Name the **immediate next step only** — `$ccgs-brainstorm [their hint]`. Do not
...
1. Ask them to describe their concept in one sentence — genre and core mechanic. Use plain text, not Codex user-input tool (it's an open response).
2. Acknowledge the concept, then use `Codex user-input tool` to offer two paths:
   - **Prompt**: "How would you like to proceed?"
   - **Options**:
     - `Formalize it first` — Run `$ccgs-brainstorm [concept]` to structure it into a proper game concept document
     - `Jump straight in` — Go to `$ccgs-setup-engine` now and write the GDD manually afterward
...
2. **Sub-case D1 — Early stage** (engine not configured or only a game concept exists):
   - Recommend `$ccgs-setup-engine` first if engine not configured
   - Then `$ccgs-project-stage-detect` for a gap inventory

   **Sub-case D2 — GDDs, ADRs, or stories already exist:**
   - Explain: "Having files isn't the same as the template's skills being able to use them. GDDs might be missing required sections. `$ccgs-adopt` checks this specifically."
   - Recommend:
     1. `$ccgs-project-stage-detect` — understand what phase and what's missing entirely
     2. `$ccgs-adopt` — audit whether existing artifacts are in the right internal format

3. Show the recommended path for D2:
   - `$ccgs-project-stage-detect` — phase detection + existence gaps
   - `$ccgs-adopt` — format compliance audit + migration plan
   - `$ccgs-setup-engine` — if engine not configured
   - `$ccgs-design-system retrofit [path]` — fill missing GDD sections
   - `$ccgs-architecture-decision retrofit [path]` — add missing ADR sections
   - `$ccgs-architecture-review` — bootstrap the TR requirement registry
   - `$ccgs-gate-check` — validate readiness for next phase
...
- If the user has **no concept at all** (Path A, or a Path B hint with nothing in
  ...
  "You can raise this after `$ccgs-brainstorm` once the concept is clearer —
  `$ccgs-settings` changes it anytime."
...
Then use `Codex user-input tool`. Order the options so the **recommended** archetype is
first and append ` (Recommended)` to its label (per the Codex user-input tool
convention); the other two follow in any order.

- **Prompt**: "What best describes what you're building? This sets how much process
  the project carries — you can change it anytime with `$ccgs-settings`."
...
team, and fewer tokens. Run `$ccgs-settings` to see the exact value each one takes, or
...
before `rigor` existed `$ccgs-start` never asked about any of them — so every project
...
**If it is not set**: Use `Codex user-input tool`:
...
still call `$ccgs-art-bible`, `$ccgs-ux-design`, `$ccgs-qa-plan` or anything else the moment you want
...
- `$ccgs-setup-engine` — configure the engine
- `$ccgs-brainstorm` — produce the one-page `design/game-brief.md` (the lean-tier design artifact; it replaces the full concept doc, systems decomposition, and per-system GDDs)
- `$ccgs-create-stories` — turn the brief's MVP list into implementable stories (the epic is implicit — no separate `$ccgs-create-epics` or `$ccgs-sprint-plan`; the brief's build order is the plan)
- `$ccgs-dev-story` — **first line of game code**

**If `standard` (default) — the full pipeline:**
- **Concept:** `$ccgs-setup-engine` → `$ccgs-brainstorm` → `$ccgs-prototype` → `$ccgs-art-bible` → `$ccgs-map-systems` → `$ccgs-design-system` (×N systems) → `$ccgs-review-all-gdds` → `$ccgs-gate-check`
- **Architecture:** `$ccgs-create-architecture` → `$ccgs-architecture-decision` (×N) → `$ccgs-create-control-manifest` → `$ccgs-architecture-review`
- **Pre-Production:** `$ccgs-ux-design` → `$ccgs-create-epics` → `$ccgs-create-stories` → `$ccgs-sprint-plan`
- **Production:** `$ccgs-dev-story`

**If `full` — the full pipeline plus validation builds:**
- Everything in `standard`, plus `$ccgs-vertical-slice` and `$ccgs-playtest-report` (×1+)
  in Pre-Production, and `$ccgs-design-review` after each GDD.
...
and offer `$ccgs-settings` to change it. Do not re-ask.

Then use `Codex user-input tool` to ask which step they'd like to take first. Never auto-run the next skill.
...
When the user confirms their next step, respond with a single short line: "Type `[skill command]` to begin." Nothing else. Do not re-explain the skill or add encouragement. The `$ccgs-start` skill's job is done.
