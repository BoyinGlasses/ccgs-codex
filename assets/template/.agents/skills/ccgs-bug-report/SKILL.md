...
description: "In a CCGS game project, structured bug report from a description, or analyze code for potential bugs. Reproduction steps, severity."
---
...
> **These labels must match `$ccgs-bug-triage`'s priority table exactly** — it parses
...
> `.agents/skills/ccgs-bug-triage/SKILL.md` in the same commit.
**Status**: Open
**Reported**: [Date]
**Reporter**: [Name]
...
If STILL PRESENT: reopen the bug, set Status back to Open, and suggest re-running `$ccgs-hotfix [BUG-ID]`.

---

## Phase 2D: Close Mode

Read `production/qa/bugs/[BUG-ID].md`. Confirm Status is `Verified Fixed` before closing. If status is anything else, stop: "Bug [ID] must be Verified Fixed before it can be closed. Run `$ccgs-bug-report verify [BUG-ID]` first."
...
After closing, check `production/qa/bug-triage-*.md` — if the bug appears in an open triage report, note: "Bug [ID] is referenced in the triage report. Run `$ccgs-bug-triage` to refresh the open bug count."
...
- Run `$ccgs-bug-triage` to prioritize alongside existing open bugs
- If S1 or S2: run `$ccgs-hotfix [BUG-ID]` for emergency fix workflow

**After fixing the bug (developer confirms fix is in):**
- Run `$ccgs-bug-report verify [BUG-ID]` — confirm the fix actually works before closing
- Never mark a bug closed without verification — a fix that doesn't verify is still Open

**After verify returns VERIFIED FIXED:**
- Run `$ccgs-bug-report close [BUG-ID]` — write the closure record and update status
- Run `$ccgs-bug-triage` to refresh the open bug count and remove it from the active list
