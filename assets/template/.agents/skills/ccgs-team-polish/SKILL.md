...
description: "In a CCGS game project, orchestrate the polish team — performance-analyst, technical-artist, sound-designer, qa-tester — to optimize and harden a feature or area."
---
...
> Usage: `$ccgs-team-polish [feature or area]` — specify the feature or area to polish (e.g., `combat`, `main menu`, `inventory system`, `level-1`). Do not use `Codex user-input tool` here; output the guidance directly.
...
> `$ccgs-team-qa` and `$ccgs-team-narrative`, whose destinations are fixed by their
...
`tests/`, and the phase is gated by an `Codex user-input tool`. A sub-agent does **not**
...
- If READY FOR RELEASE: run `$ccgs-release-checklist` for the final pre-release validation.
- If NEEDS MORE WORK: schedule remaining issues in `$ccgs-sprint-plan update` and re-run `$ccgs-team-polish` after fixes.
- Run `$ccgs-gate-check` for a formal phase gate verdict before handing off to release.
