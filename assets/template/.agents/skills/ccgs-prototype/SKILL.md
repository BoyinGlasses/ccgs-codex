---
name: ccgs-prototype
description: "In a CCGS game project, concept prototype before GDDs — throwaway HTML, Engine or Paper build, PROCEED/PIVOT/KILL. After $ccgs-brainstorm and $ccgs-setup-engine."
---
...
**Default use** — run right after `$ccgs-brainstorm` and `$ccgs-setup-engine`, before writing
...
before committing to Production, run `$ccgs-vertical-slice` instead.
...
Otherwise, use `Codex user-input tool` to confirm intent before proceeding:
...
[answer]." Suggest next step: `$ccgs-map-systems` or `$ccgs-design-system [mechanic]`.
...
Assess which path best fits the hypothesis, then use `Codex user-input tool` with your
...
1. `$ccgs-design-review design/gdd/game-concept.md` — validate the concept doc against what the prototype revealed
2. `$ccgs-gate-check` — confirm readiness to advance to Systems Design
3. `$ccgs-art-bible` — define visual identity (optional but worth doing before GDDs)
4. `$ccgs-map-systems` — decompose the concept into all game systems
5. `$ccgs-design-system [mechanic]` — GDD for each MVP system; use prototype learnings
   in the Tuning Knobs and Formulas sections
6. `$ccgs-review-all-gdds` — cross-system consistency check
...
the revised hypothesis for the next prototype. When `$ccgs-prototype` is next run, check
...
- Run `$ccgs-prototype [revised-concept]` to test the adjusted direction
- Or `$ccgs-brainstorm [hint]` if the concept needs more fundamental rethinking
...
- Run `$ccgs-brainstorm open` or `$ccgs-brainstorm [new-hint]` to explore a different concept
...
2. **Choose path** — same Codex user-input tool widget as Phase 3 (HTML / Engine / Paper).
