...
description: "In a CCGS game project, analyze project state, detect stage, identify gaps, recommend next steps. 'Where are we in development?'"
---
...
At runtime, execute this shell snippet if needed: `source "${CLAUDE_PROJECT_DIR:-.}/.claude/hooks/yaml-helper.sh" 2>/dev/null && resolve_config --keys workflow,automation`
...
Based on scanned artifacts, determine stage. Check `project.stage` in `project.yaml` first (if present); else `production/stage.txt` (legacy fallback) — either is an explicit override from `$ccgs-gate-check`. Otherwise, auto-detect using these heuristics (check from most-advanced backward):
...
| **Polish** | Explicit only (set by `$ccgs-gate-check` Production → Polish gate) |
| **Release** | Explicit only (set by `$ccgs-gate-check` Polish → Release gate) |
...
- "I found a game concept but no systems index. Have you decomposed the concept into individual systems yet, or should we run `$ccgs-map-systems`?"
...
If user provided a role argument (e.g., `$ccgs-project-stage-detect programmer`):
...
$ccgs-project-stage-detect programmer

# Designer-focused analysis
$ccgs-project-stage-detect designer
```
...
suggest authoring optional docs (e.g. don't suggest `$ccgs-reverse-document` for an
absent GDD at `minimal`, where code is the expected next step):

- **Concept exists but no systems index?** → `$ccgs-map-systems` to decompose into systems
- **Missing design docs?** → `$ccgs-reverse-document design src/[system]`
- **Missing architecture docs?** → `$ccgs-architecture-decision` or `$ccgs-reverse-document architecture`
- **Prototypes need documentation?** → `$ccgs-reverse-document concept prototypes/[name]`
- **No sprint plan?** → `$ccgs-sprint-plan`
- **Approaching milestone?** → `$ccgs-milestone-review`
