---
name: ccgs-dev-story
description: "In a CCGS game project, implement a story: ADR guidelines, right programmer agent, code plus test. After /story-readiness, before /code-review and /story-done."
---
...
At runtime, execute this shell snippet if needed: `source "${CLAUDE_PROJECT_DIR:-.}/.claude/hooks/yaml-helper.sh" 2>/dev/null && resolve_config --keys automation,workflow,story_granularity,qa.level,testing.strict,system_overrides`
...
```
$ccgs-qa-plan sprint           ← define test requirements before sprint begins
$ccgs-story-readiness [path]   ← validate before starting
$ccgs-dev-story [path]         ← implement it  (this skill)
...
|------|------|---------------------|-----------|-----------|
| TR registry | `docs/architecture/tr-registry.yaml` | **STOP** — "TR registry not found at `docs/architecture/tr-registry.yaml`. Run `$ccgs-architecture-review` to bootstrap the registry from your GDDs and ADRs." | optional — proceed without it | not expected — proceed |
...
**Do not open the ADR by default.** `$ccgs-create-stories` already distilled it into
...
| Grep returns **no match** and the story reads `unversioned` | The ADR carries no `## Last Verified`. Consistent, not stale. | **Trust the story**; recommend `$ccgs-architecture-decision [file] retrofit` to add the field. |
...
staleness gates that already exist (`$ccgs-story-readiness` on Manifest Version,
`$ccgs-code-review` post-implementation) are what make that safe.

**On mismatch**, use `Codex user-input tool` — same shape as the Manifest Version
check below:
...
If they differ, use `Codex user-input tool` before proceeding:
...
`specialists.*`; the five skills that already consume it (`$ccgs-adopt`,
`$ccgs-code-review`, `$ccgs-design-system`, `$ccgs-gate-check`, `$ccgs-team-ui`) all read the block
...
`$ccgs-setup-engine` writes the block, and it is the project's own answer to "which
...
> and one where `$ccgs-architecture-review` was supposed to bootstrap it and nobody
...
> `testing.strict.*` is locally overridable (`$ccgs-settings --local
...
unverifiable — test required before `$ccgs-story-done`". At an ADVISORY level it is
...
4. **Run it and look.** A parse check is not a run. For every story that
   ...
   **`NOT VERIFIED` is a blocker at the default gate level for Visual/Feel and
   UI, not a note** — `$ccgs-story-done` reads this line. This step is **not waived
...
```
## Session Extract — $ccgs-dev-story [date]
...
- A *referenced* ADR's status is Proposed → do not implement; run `$ccgs-architecture-decision` first (at `standard`/`minimal` a story that references no ADR is not blocked on this — see Phase 2)
...
- **Visual/Feel criteria are deferred, not skipped** — mark them as DEFERRED
  in the summary; they will be manually verified in `$ccgs-story-done`
