...
description: "In a CCGS game project, traceability matrix mapping GDD requirements to ADRs. Finds gaps, cross-ADR conflicts, engine compatibility. PASS/CONCERNS/NOT ASSESSED/FAIL."
---
...
At runtime, execute this shell snippet if needed: `source "${CLAUDE_PROJECT_DIR:-.}/.claude/hooks/yaml-helper.sh" 2>/dev/null && resolve_config --keys automation,workflow`
# Architecture Review
...
- **Everything `UNCHANGED`** (and no `UNRESOLVED`) — nothing this review
  ...
  prior report's date and verdict and offer via `Codex user-input tool`: `[A] Stand
...
- **Some `CHANGED`/`NEW`** — name them, then scope instead of re-running
  everything: recommend `$ccgs-architecture-review [system]` (single-system mode)
...
predate `## Summary` (`$ccgs-design-system` emits it, but older GDDs lack it) — never
...
| **0 matches, N_adr > 0** | **Malformed ADRs**, not "no architecture". | "[N_adr] ADRs found, none carries a scannable section — run `$ccgs-architecture-decision [file] retrofit` on each." Do **not** report zero coverage; that would read as a design failure when it is a format failure. |
...
> route out — `$ccgs-architecture-decision accept ADR-NNNN`. That route is the only
...
read of each story** — the same two-grep form `$ccgs-test-evidence-review` uses for
...
Then use `Codex user-input tool`:
...
If [B]: display the complete proposed systems-index section, then re-ask with `Codex user-input tool`.
...
For each gap:
  ❌ TR-[id]: [GDD] → [system] → [requirement]
     Suggested ADR: "$ccgs-architecture-decision [suggested title]"
...
Use `Codex user-input tool` for the write approval:
...
For `rtm` mode, use `Codex user-input tool`:
...
> Last Updated: [date]
> Mode: $ccgs-architecture-review rtm
...
```markdown
### [YYYY-MM-DD] — $ccgs-architecture-review — 🔴 CONFLICT
...
`production/session-state/active.md`:
    ...
    ## Session Extract — $ccgs-architecture-review [date]
...
2. **Pre-gate checklist**: Check whether these exist via Glob and mark each ✅ or ❌:
   - `tests/unit/` and `tests/integration/` directories — if ❌: run `$ccgs-test-setup`
   - `.github/workflows/tests.yml` — if ❌: run `$ccgs-test-setup`
   - `design/accessibility-requirements.md` — if ❌: run `$ccgs-ux-design`
   - `design/ux/interaction-patterns.md` — if ❌: run `$ccgs-ux-design`
   Present ❌ items as required steps before gate-check. Do not offer `$ccgs-gate-check`
   as an option if any item is ❌ — offer the missing skill to run instead.
3. **Rerun trigger**: "Re-run `$ccgs-architecture-review` after each new ADR is written
   to verify coverage improves"

Then close with `Codex user-input tool` tailored to the pre-gate checklist state:
- If ADR gaps remain or any pre-gate item is ❌:
  - "Architecture review complete. What would you like to do next?"
    - [A] Write a missing ADR — open a fresh session and run `$ccgs-architecture-decision [system]`
    - [B] Run `$ccgs-test-setup` — required before gate-check (only show if test infrastructure is ❌)
    - [C] Run `$ccgs-ux-design` — required before gate-check (only show if UX/accessibility files are ❌)
...
- If all pre-gate checklist items are ✅ and no blocking ADR gaps remain:
  - "Architecture review complete. All pre-gate items confirmed. What would you like to do next?"
    - [A] Run `$ccgs-gate-check pre-production`
    - [B] Write a missing ADR — open a fresh session and run `$ccgs-architecture-decision [system]`
