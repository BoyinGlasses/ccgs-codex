...
description: "In a CCGS game project, generate missing design or architecture docs from existing implementation — works backwards from code and prototypes."
---
...
**Format**: `$ccgs-reverse-document <type> <path>`
...
At runtime, execute this shell snippet if needed: `source "${CLAUDE_PROJECT_DIR:-.}/.claude/hooks/yaml-helper.sh" 2>/dev/null && resolve_config --keys workflow,system_overrides,automation`
...
**Examples**:
```bash
$ccgs-reverse-document design src/gameplay/magic-system
$ccgs-reverse-document architecture src/core/entity-component
$ccgs-reverse-document concept prototypes/vehicle-combat
```
...
> If the design exists only in your head, `$ccgs-design-system [name]` is the skill
...
> a report — it is a **design document**, and `$ccgs-design-review`, `$ccgs-create-epics` and
> `$ccgs-create-stories` will read it as a statement of authored intent. A fabricated
...
identically**. `$ccgs-design-review` checks it for completeness, `$ccgs-create-epics`
derives epics from it, `$ccgs-create-stories` turns its lines into acceptance
...
```markdown
> **Reverse-documented from implementation** — generated by `$ccgs-reverse-document`
...
1. Run $ccgs-balance-check on combat formulas (exponential scaling concern)
...
```
User: $ccgs-reverse-document design src/gameplay/[system]
...
Agent: ✅ Written to design/gdd/[system-name].md
       ...
       - Run $ccgs-balance-check to validate [curve]
