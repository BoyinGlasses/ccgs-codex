...
description: "In a CCGS game project, author the architecture blueprint before code is written. Validates decisions against the pinned engine, flags knowledge gaps."
---
...
**Distinct from `$ccgs-architecture-decision`**: ADRs record individual point decisions.
...
> "No engine is configured. Run `$ccgs-setup-engine` first. Architecture cannot be
...
- **`systems-index.md` absent** — stop:
  > "No systems index found. Run `$ccgs-map-systems` first. An architecture written
...
- **`game-concept.md` absent** — at `standard`/`full`, stop and point at
  `$ccgs-brainstorm`. At `minimal`, read `design/game-brief.md` in its place; if that
...
- `$ccgs-architecture-decision [title]` → covers: TR-[id], TR-[id]

**Core Layer:**
- `$ccgs-architecture-decision [title]` → covers: TR-[id]
...
Display a one-paragraph summary of what the document will contain (layers, modules, data flows, ADR gaps). Then use `Codex user-input tool`:
...
Use `Codex user-input tool` — "Technical Director and Lead Programmer have reviewed the architecture. How would you like to proceed?"
...
Show the proposed Document Status block inline, then use `Codex user-input tool`:
...
**1. `$ccgs-architecture-decision "[Title]"` → ADR-[XXXX]**
[One sentence: what it defines and what it unblocks.]

**2. `$ccgs-architecture-decision "[Title]"` → ADR-[XXXX]**
[One sentence.]

**3. `$ccgs-architecture-decision "[Title]"` → ADR-[XXXX]**
[One sentence.]
...
> **Required before `$ccgs-gate-check [stage]`:**
> - [ ] Accept ADRs: [list Proposed ADR IDs that must be Accepted]
> - [ ] Write ADRs: [list ADR IDs that must still be written]
> - [ ] Run `$ccgs-test-setup` — scaffolds `tests/unit/`, `tests/integration/`, CI workflow, and an example test file
> - [ ] Run `$ccgs-ux-design` — creates `design/ux/interaction-patterns.md` and `design/accessibility-requirements.md`
>
> Run `$ccgs-gate-check [stage]` when all boxes are checked.

If nothing is blocking, write instead:
> No blockers — run `$ccgs-gate-check [stage]` now.
...
5. **Use `Codex user-input tool` for write approvals** — plain text "May I?" is not
...
- Run `$ccgs-architecture-decision [title]` for each required ADR listed in Phase 6 — Foundation layer ADRs first
- Run `$ccgs-architecture-review` — bootstraps the Requirements Traceability Matrix and TR registry from the ADRs just written. Required before the Pre-Production gate.
- Run `$ccgs-test-setup` to scaffold `tests/unit/`, `tests/integration/`, CI workflow, and an example test (required for gate-check)
- Run `$ccgs-ux-design` to initialize `design/ux/interaction-patterns.md` and `design/accessibility-requirements.md` (required for gate-check)
- Run `$ccgs-create-control-manifest` once the required ADRs are written to produce the layer rules manifest
- Run `$ccgs-gate-check pre-production` when all required ADRs, `$ccgs-test-setup`, and `$ccgs-ux-design` are complete
