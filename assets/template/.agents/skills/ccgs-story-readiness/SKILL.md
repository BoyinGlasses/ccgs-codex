...
description: "In a CCGS game project, is a story implementation-ready? Checks clear acceptance criteria, open questions, ADR refs. READY/NEEDS WORK/BLOCKED/NOT ASSESSED."
---
...
**Scope:** `$ARGUMENTS[0]` (blank = ask user via Codex user-input tool)

- **Specific path** (e.g., `$ccgs-story-readiness production/epics/combat/story-001-basic-attack.md`):
...
If no argument is given, use `Codex user-input tool`:
...
> list, or the specific path given), and route: `$ccgs-create-epics [layer]` then
...
- All ADR status fields — resolve these with **one scan, not one read per ADR**.
  ...
  "no Accepted ADRs"** — report "run `$ccgs-architecture-decision [file] retrofit`"
...
- [ ] **TR-ID is valid and active**: If the story contains a `TR-[system]-NNN`
  ...
  - If the ID does not exist in the registry → NEEDS WORK: ID was not registered
    (story may predate registry, or registry needs an `$ccgs-architecture-review` run).
...
Resolve these before the sprint begins or replan with `$ccgs-sprint-plan update`.
```
...
- If a story file does not exist at all: "This story file is missing entirely.
  Run `$ccgs-create-epics [layer]` then `$ccgs-create-stories [epic-slug]` to generate stories from the GDD and ADR."
- If a story has no GDD reference and the work appears small: "This story has
  ...
  `$ccgs-quick-design [description]` to create a Quick Design Spec, then reference
...
Run `$ccgs-story-readiness [path]` to validate before starting.
```
...
- **GAPS [list]** → surface the specific gaps to the user via `Codex user-input tool`:
...
- Run `$ccgs-dev-story [story-path]` to begin implementation once the story is READY
- Run `$ccgs-story-readiness sprint` to check all stories in the current sprint at once
- Run `$ccgs-create-stories [epic-slug]` if a story file is missing entirely
