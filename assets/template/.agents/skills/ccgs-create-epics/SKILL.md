---
name: ccgs-create-epics
description: "In a CCGS game project, turn GDDs plus architecture into epics — one per architectural module, with untraced requirements. Then $ccgs-create-stories [epic-slug]."
---
...
At runtime, execute this shell snippet if needed: `source "${CLAUDE_PROJECT_DIR:-.}/.claude/hooks/yaml-helper.sh" 2>/dev/null && resolve_config --keys review_mode,automation,workflow,docs.density,story_granularity,system_overrides`
...
**Next step after each epic:** `$ccgs-create-stories [epic-slug]`

**When to run:** After `$ccgs-create-control-manifest` and `$ccgs-architecture-review` pass
(at `full`). At `standard`, critical ADRs + a control manifest suffice. At
`minimal`, this skill is **optional and not part of the path** — `$ccgs-create-stories`
...
> **At `minimal`, this skill is optional — `$ccgs-create-stories` synthesizes the epic**
> (Option A). At that tier `$ccgs-create-stories` reads `design/game-brief.md`
> directly, writes a lightweight implicit `production/epics/<slug>/EPIC.md`, and
> generates stories from the MVP list — so the path is `$ccgs-brainstorm` →
> `$ccgs-create-stories` → `$ccgs-dev-story`, with no separate `$ccgs-create-epics` or
> `$ccgs-sprint-plan` step. **At `standard`/`full` an epic IS required**:
> `$ccgs-create-stories` reads `production/epics/[slug]/EPIC.md`, and skipping this
> skill there leaves `$ccgs-dev-story` with no story to implement — a dead end, not a
> shortcut.
...
**Modes:**
- `$ccgs-create-epics all` — process all systems in layer order
- `$ccgs-create-epics layer: foundation` — Foundation layer only
- `$ccgs-create-epics layer: core` — Core layer only
- `$ccgs-create-epics layer: feature` — Feature layer only
- `$ccgs-create-epics layer: presentation` — Presentation layer only
- `$ccgs-create-epics [system-name]` — one specific system
...
in `design/gdd/` — run `$ccgs-design-system` first" and stop.
...
| **0 matches, N > 0** | Expected for GDDs authored before `$ccgs-design-system` emitted `## Summary`. | Determine scope for **all N** from `systems-index.md` and full-read the in-scope set. This is the pre-optimization behaviour — correct, only more expensive. Note once: "No `## Summary` sections found across [N] GDDs — scoping from `systems-index.md` instead of Summary." |
...
>   produced — but note `$ccgs-create-stories` also synthesizes one from the brief when
...
> **At `minimal` there is no `systems-index.md`** — `$ccgs-map-systems` is not required
...
> exist first. Do not stop, and do not send the user to `$ccgs-map-systems` to satisfy
...
> Run `$ccgs-architecture-decision` first, or proceed with placeholders."
...
If CONCERNS, use `Codex user-input tool`:
...
> **Status**: Ready
> **Stories**: Not yet created — run `$ccgs-create-stories [epic-slug]`
...
- All stories are implemented, reviewed, and closed via `$ccgs-story-done`
...
Run `$ccgs-create-stories [epic-slug]` to break this epic into implementable stories.
```
...
- **Foundation + Core complete**: These are required for the Pre-Production →
  Production gate. Run `$ccgs-gate-check production` to check readiness.
- **Reminder**: Epics define scope. Stories define implementation steps. Run
  `$ccgs-create-stories [epic-slug]` for each epic before developers can pick up work.
...
- **Verdict: COMPLETE** — [N] epic(s) written. Run `$ccgs-create-stories [epic-slug]` per epic.
