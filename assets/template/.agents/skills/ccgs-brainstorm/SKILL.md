...
description: "In a CCGS game project, guided concept ideation using professional studio techniques, player psychology, creative exploration."
---
...
At runtime, execute this shell snippet if needed: `source "${CLAUDE_PROJECT_DIR:-.}/.claude/hooks/yaml-helper.sh" 2>/dev/null && resolve_config --keys review_mode,automation,docs.density,workflow`
...
1. **Concept** — from the argument hint (or one quick open question if none), propose
   **2–3 one-line concepts** (verb-first or mashup, per Phase 2's techniques). One
   `Codex user-input tool` to pick or combine — the only guaranteed creative prompt.
...
3. **Write approval** — `Codex user-input tool`: "Brief is ready. May I write it to
...
4. **Next steps** (short — this is the point). **Read `engine.name` from
   `project.yaml` first and list `$ccgs-setup-engine` only if it is absent or empty:**
   1. *(only when `engine.name` is absent or empty)* "`$ccgs-setup-engine` — configure
      the engine (required before code)"
   2. "`$ccgs-create-stories` — turn the brief's MVP list into implementable stories. The
      epic is implicit at `minimal`; there is no separate `$ccgs-create-epics` or
      `$ccgs-sprint-plan` — the brief's build order is the plan."
   3. "`$ccgs-dev-story` — first line of game code"

   > **Why the condition.** `$ccgs-start`'s `minimal` path runs `$ccgs-setup-engine`
   > *before* `$ccgs-brainstorm`, and `$ccgs-setup-engine` Section 2 states that a missing
...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use `Codex user-input tool`** at key decision points throughout brainstorming:
   ...
   `Codex user-input tool` to capture the decision with concise labels.
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text — the user must be able to type specific game names freely.
  Do NOT put this in an Codex user-input tool with preset options.)*
- Are there genres you love? Genres you avoid? Why?
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? *(Use `Codex user-input tool` for this — constrained choice.)*

**Practical constraints** (shape the sandbox before brainstorming).
Bundle these into a single multi-tab `Codex user-input tool` with these exact tab labels:
...
Present all three. Then use `Codex user-input tool` to capture the selection.

**This decision asks regardless of `modes.automation`.** Concept selection
is the creative heart of `$ccgs-brainstorm` and has no defensible autonomous
...
Ask these as `Codex user-input tool` calls — derive the options from the chosen concept, don't hardcode them:
...
**Pillar confirmation**: After presenting the full pillar set, use `Codex user-input tool`:
...
If the user selects B, C, or D, make the revision, then use `Codex user-input tool` again:
...
Collect both verdicts, then present them together using a two-tab `Codex user-input tool`:
...
- **Target platform**: Use `Codex user-input tool` — "What platforms are you targeting for this game?"
  Options: `PC (Steam / Epic)` / `Mobile (iOS / Android)` / `Console` / `Web / Browser` / `Multiple platforms`
  Record the answer — it directly shapes the engine recommendation and will be passed to `$ccgs-setup-engine`.
...
- **Engine experience**: Use `Codex user-input tool` — "Do you already have an engine you work in?"
  ...
  - If "No preference" → tell them: "Run `$ccgs-setup-engine` after this session — it will walk you through the full decision based on your concept and platform target." Do not make a recommendation here.
...
5. Use `Codex user-input tool` for write approval:
...
If [B]: ask which section to revise using `Codex user-input tool` with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else — I'll describe`

After revising, show the updated section as a diff or clear before/after, then use `Codex user-input tool` — "Ready to write the updated concept document?"
...
**Path A — Design-First** (recommended if the concept is well-defined):
   1. "Run `$ccgs-setup-engine` to configure the engine and populate version-aware reference docs"
   2. "Run `$ccgs-art-bible` to create the visual identity specification — do this BEFORE writing GDDs. **The art bible is required before the Technical Setup gate.** It gates asset production and shapes technical architecture decisions (rendering, VFX, UI systems)."
   3. "Use `$ccgs-design-review design/gdd/game-concept.md` to validate concept completeness before going downstream"
   4. "Discuss vision with the `creative-director` agent for pillar refinement"
   5. "Decompose the concept into individual systems with `$ccgs-map-systems` — maps dependencies, assigns priorities, and creates the systems index"
   6. "Author per-system GDDs with `$ccgs-design-system` — guided, section-by-section GDD writing for each system identified in step 5"
   7. "Plan the technical architecture with `$ccgs-create-architecture` — produces the master architecture blueprint and Required ADR list"
   8. "Record key architectural decisions with `$ccgs-architecture-decision (×N)` — write one ADR per decision in the Required ADR list from `$ccgs-create-architecture`"
   9. "Run `$ccgs-architecture-review` — bootstraps the TR registry and Requirements Traceability Matrix from your GDDs and ADRs (required before the Pre-Production gate)"
   10. "Validate readiness to advance with `$ccgs-gate-check` — phase gate before committing to production"

**Path B — Prototype-First** (use if the core mechanic is unproven or the concept needs validation):
   1. "Run `$ccgs-setup-engine` to configure the engine"
   2. "Run `$ccgs-prototype [core-mechanic]` — validate the core idea is fun before writing any GDDs (1–3 days throwaway code)"
   3. "If prototype PROCEEDS: run `$ccgs-art-bible`, then continue with Path A steps 5–10 above, using prototype learnings to inform your GDDs"
   4. "If prototype PIVOTS: return to `$ccgs-brainstorm` with the learnings and reshape the concept"
   5. "After full design and architecture, build the `$ccgs-vertical-slice` to validate production readiness before committing to sprints"
...
1. `$ccgs-setup-engine` — configure the engine and populate version-aware reference docs
2. `$ccgs-art-bible` — establish visual identity before writing any GDDs
3. `$ccgs-map-systems` — decompose the concept into individual systems with dependencies
4. `$ccgs-design-system [first-system]` — author per-system GDDs in dependency order
5. `$ccgs-create-architecture` — produce the master architecture blueprint
6. `$ccgs-architecture-review` — bootstrap TR registry and Requirements Traceability Matrix
7. `$ccgs-gate-check pre-production` — validate readiness before committing to production
