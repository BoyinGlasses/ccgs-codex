...
description: "In a CCGS game project, validate skill files for structural compliance and behavioral correctness. Four modes: static linter, spec, category rubric, audit."
---
...
`project.yaml` → default `collaborative`). Every `Codex user-input tool` call and
...
|------|---------|---------|------------|
| `static` | `$ccgs-skill-test static [name\|all]` | Structural linter — 7 compliance checks per skill | Low (~1k/skill) |
| `spec` | `$ccgs-skill-test spec [name]` | Behavioral verifier — evaluates assertions in test spec | Medium (~5k/skill) |
| `category` | `$ccgs-skill-test category [name\|all]` | Category rubric — checks skill against its category-specific metrics | Low (~2k/skill) |
| `audit` | `$ccgs-skill-test audit` | Coverage report — skills, agent specs, last test dates | Low (~3k total) |
...
or a value rather than a judgement. `$ccgs-settings` is the reference case: it prints
...
> for reasons that were not their fault — `$ccgs-prototype` and `$ccgs-vertical-slice`
> advertise `PROCEED`/`PIVOT`/`KILL` in their own descriptions, `$ccgs-adopt` and
> `$ccgs-security-audit` rank by severity, and `$ccgs-settings` has no verdict by design.
...
- A final section mentioning another skill (e.g., `$ccgs-story-done`, `$ccgs-gate-check`)
...
- Spec file not found at path: "Spec file missing at [path]. Run `$ccgs-skill-test audit`
...
Offer: "Would you like to run `$ccgs-skill-test static all` to check structural
compliance across all skills? `$ccgs-skill-test category all` to run category rubric
checks? Or `$ccgs-skill-test spec [name]` to run a specific behavioral test?"
...
- After `static [name]`: "Run `$ccgs-skill-test spec [name]` to validate behavioral
  correctness if a test spec exists."
- After `static all` with failures: "Address NON-COMPLIANT skills first. Run
  `$ccgs-skill-test static [name]` individually for detailed remediation guidance."
- After `spec [name]` PASS: "Update `CCGS Skill Testing Framework/catalog.yaml` to record this
  pass date. Consider running `$ccgs-skill-test audit` to find the next spec gap."
