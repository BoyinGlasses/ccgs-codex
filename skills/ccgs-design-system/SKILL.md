...
description: "In a CCGS game project, section-by-section GDD authoring for one system — walks through each required section, cross-references dependencies."
---
...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use `Codex user-input tool`:
...
3. If no systems index exists, fail with:
   > "Usage: `$ccgs-design-system <system-name>` — e.g., `$ccgs-design-system movement`
   > Or to fill gaps in an existing GDD: `$ccgs-design-system retrofit design/gdd/[system-name].md`
   > No systems index found. Run `$ccgs-map-systems` first to map your systems and get the design order."
...
one section `$ccgs-review-all-gdds` and the tiered-loading readers depend on. Read
...
- `standard` — Overview, Detailed Design, Edge Cases, Dependencies, Acceptance
  ...
  **defines numeric rules**: rates, curves, thresholds, costs, damage, drop
  ...
  > those four tokens are not what `$ccgs-map-systems` writes — `templates/systems-index.md`
...
> index to hold them: `$ccgs-map-systems` owns that file (§5d).
>
...
> Run `$ccgs-brainstorm` first"* — killing the escape hatch on step one of the very
> configuration it was built for.

- **Game concept**: Read `design/gdd/game-concept.md` — fail if missing:
  > "No game concept found. Run `$ccgs-brainstorm` first."
- **Systems index**: Read `design/gdd/systems-index.md` — fail if missing:
  > "No systems index found. Run `$ccgs-map-systems` first to map your systems."
...
> `$ccgs-setup-engine` before moving to architecture if you haven't already."
...
> "Verdict: **BLOCKED** — skeleton creation declined. The design session cannot proceed without the skeleton file, as all subsequent phases use it as the base. Re-run `$ccgs-design-system [system]` when ready to create the file."
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   `Codex user-input tool` for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use `Codex user-input tool` to capture the decision.
...
6. **Approval**: Per the resolved `modes.automation` mode
   ...
   **In `collaborative` mode**: Immediately after the draft — in the SAME
   response — use `Codex user-input tool`. **NEVER use plain text. NEVER skip
   this step.**
...
**Framing questions (ask BEFORE drafting)**: Use `Codex user-input tool` with a multi-tab widget:
...
in `$ccgs-architecture-decision`, not the GDD. The GDD describes behavior; the ADR
...
**Framing question (ask BEFORE drafting)**: Use `Codex user-input tool`:
...
- Surface any disagreements between agents to the user via `Codex user-input tool`
...
- Present the specialists' proposals to the user for review via `Codex user-input tool`
...
> `$ccgs-map-systems` assigns to combat, AI and movement — the largest bucket in most
...
other GDD"* rather than leaving the placeholder. `$ccgs-review-all-gdds` Phase 2c
...
> "📌 **Asset Spec** — Visual/Audio requirements are defined. After the art bible is approved, run `$ccgs-asset-spec system:[system-name]` to produce per-asset visual descriptions, dimensions, and generation prompts from this section."
...
> (Pre-Production), run `$ccgs-ux-design` to create a UX spec for each screen or
...
(`$ccgs-create-epics`, `$ccgs-architecture-review`, `$ccgs-review-all-gdds` all grep it), so a
...
> `$ccgs-design-review design/gdd/[system-name].md`
>
> **Never run `$ccgs-design-review` in the same session as `$ccgs-design-system`.** The reviewing
...
**NEVER offer to run `$ccgs-design-review` inline.** Always direct the user to a fresh window.
...
- **Absent** — say so in one line (*"no `design/gdd/systems-index.md` at this
  ...
  one: `$ccgs-map-systems` owns that file, and a stub written here would be a systems
...
- "What's next?"
  - Options:
    - "Run `$ccgs-consistency-check` — verify this GDD's values don't conflict with existing GDDs (recommended before designing the next system)"
    - "Design next system ([next-in-order])" — if undesigned systems remain
    - "Fix review findings" — if design-review flagged issues
    - "Stop here for this session"
    - "Run `$ccgs-gate-check`" — if enough MVP systems are designed
...
- The main session presents the agent's output to the user via `Codex user-input tool`
- The user decides; the main session writes to file
...
2. **Codex user-input tool** at every decision point (Explain -> Capture pattern):
   ...
   - Phase 5: "May I update the entity registry? May I update the systems
     ...
     `$ccgs-design-review` inline, because the reviewing agent must not inherit this
...
> open a fresh Codex session and run `$ccgs-design-system [system-name]` — it will
...
- Run `$ccgs-design-review design/gdd/[system-name].md` in a **fresh session** to validate the completed GDD independently
- Run `$ccgs-consistency-check` to verify this GDD's values don't conflict with other GDDs
- Run `$ccgs-map-systems next` to move to the next highest-priority undesigned system
- Run `$ccgs-gate-check pre-production` when all MVP GDDs are authored and reviewed
