...
description: "In a CCGS game project, generate engine-specific test helper libraries — assertion utilities, factory functions, mocks in tests/helpers/. Reduces boilerplate."
---
...
**When to run:**
- After `$ccgs-test-setup` scaffolds the framework (first time)
...
**Modes:**
- `$ccgs-test-helpers [system-name]` — generate helpers for a specific system
  (e.g., `$ccgs-test-helpers combat`)
- `$ccgs-test-helpers all` — generate helpers for all systems with test files
- `$ccgs-test-helpers scaffold` — generate only the base helper library (no
...
If the engine is not configured in either source: "Engine not configured. Run `$ccgs-setup-engine` first."
...
## Generated by $ccgs-test-helpers combat on [date].
...
- Run `$ccgs-test-setup` if the test framework has not been scaffolded yet.
- Use `$ccgs-dev-story` to implement stories — helpers reduce boilerplate in new test files.
- Run `$ccgs-skill-test` to validate other skills that may need helper coverage.
