...
description: "In a CCGS game project, orchestrate the release team — release-manager, qa-lead, devops-engineer, producer — to execute a release from candidate to deployment."
---
...
2. If a version is found: report "No version argument provided — inferred [version] from milestone data. Proceeding." Then confirm with `Codex user-input tool`: "Releasing [version]. Is this correct?"
3. If no version is discoverable: use `Codex user-input tool` to ask "What version number should be released? (e.g., v1.0.0)" and wait for user input before proceeding. Do NOT default to a hardcoded version string.
...
| 2 release-manager | `production/releases/release-checklist-[version].md` | `$ccgs-release-checklist` |
...
| 6 community-manager | `docs/patch-notes/[version].md` | `$ccgs-patch-notes` |
...
> `production/releases/` is where `$ccgs-release-checklist` already writes, and
> `docs/patch-notes/[version].md` is where `$ccgs-patch-notes` already writes, so the
...
- Generate the release checklist using `$ccgs-release-checklist`
...
- Use `Codex user-input tool` with options:
...
Before tagging or deploying, use `Codex user-input tool`:
...
- Generate changelog using `$ccgs-changelog`
- Deploy to staging for final smoke test
- Deploy to production (only if the user approved production above)
- Human team action: Monitor dashboards and error rates for 48 hours post-release. Schedule a follow-up retrospective using `$ccgs-retrospective` at the 48-hour mark.

Delegate to **community-manager** (in parallel with deployment):
- Finalize patch notes using `$ccgs-patch-notes [version]`
...
- **Sub-agents spawned via `Agent`** follow the **bounded exception** documented above
  ...
  is gated by an `Codex user-input tool`. A sub-agent does **not** prompt per write inside
...
- Run `$ccgs-retrospective` if significant issues occurred during the release.
