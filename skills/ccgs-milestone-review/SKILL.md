...
description: "In a CCGS game project, milestone progress review — completeness, quality metrics, risk, go/no-go recommendation. At checkpoints or before a deadline."
---
...
> **These alternates are copied from `$ccgs-sprint-plan`'s emitted headings — keep
...
> Retrospective`, none of which `$ccgs-sprint-plan` writes (it emits `## Sprint
...
If OFF TRACK, use `Codex user-input tool` before generating the recommendation:
...
If AT RISK, use `Codex user-input tool`:
...
- Run `$ccgs-gate-check` for a formal phase gate verdict if this milestone marks a development phase boundary.
- Run `$ccgs-sprint-plan` to adjust the next sprint based on the scope recommendations above.
