---
name: ccgs-retrospective
description: "In a CCGS game project, sprint or milestone retrospective from completed work, velocity, blockers. Actionable insights for the next iteration."
---
...
If a matching file is found, use `Codex user-input tool`:
...
> "No sprint data found for [sprint/milestone]. Run `$ccgs-sprint-status` to generate
> sprint data first, or provide the sprint details manually."

Then use `Codex user-input tool` to present two options:
...
If the user selects [A]: Proceed to invoke `$ccgs-sprint-plan new`, passing the retrospective file path and a summary of the action items and velocity change so the sprint planner can reference them.

- If this was a milestone retrospective, run `$ccgs-gate-check` to formally assess readiness for the next phase.
