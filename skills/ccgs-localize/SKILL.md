---
name: ccgs-localize
description: "In a CCGS game project, localization pipeline — find hardcoded strings, extract string tables, cultural review, VO, RTL, enforce string freeze."
---
...
### Issues
- [N] hardcoded strings found in source code (run $ccgs-localize scan)
...
[ ] $ccgs-localize scan shows zero hardcoded strings
[ ] $ccgs-localize validate shows no placeholder mismatches in source (en)
...
## Post-Freeze Changes
[Any strings added or modified after freeze are listed here automatically by $ccgs-localize extract]
```
...
before any locale ships. This is not the same as `$ccgs-localize validate` (which checks
...
- The list of all screens/flows in the game (from `design/gdd/` or `$ccgs-content-audit` output)
- The current `$ccgs-localize validate` report
...
```
$ccgs-localize scan            → find hardcoded strings
$ccgs-localize extract         → build string table
$ccgs-localize freeze          → lock source before sending to translators
$ccgs-localize brief           → generate translator briefing document
[Send to translators]
$ccgs-localize validate        → check returned translations
$ccgs-localize cultural-review → flag culturally sensitive content
$ccgs-localize rtl-check       → if shipping Arabic / Hebrew / Persian
$ccgs-localize vo-pipeline     → if shipping dubbed VO
$ccgs-localize qa              → full localization QA pass
```

After `qa` returns PASS for all shipping locales, include the QA report path when running `$ccgs-gate-check release`.
