---
name: ccgs-propagate-design-change
description: "In a CCGS game project, a GDD changed — scan ADRs and the traceability index for now-stale architectural decisions. Impact report, guides resolution."
---
...
**Usage:** `$ccgs-propagate-design-change design/gdd/combat-system.md`
...
> "Usage: `$ccgs-propagate-design-change design/gdd/[system].md`
...
an empty stub until `$ccgs-design-system` populates it), skip this — the ADR cascade
below still runs.
...
- **CONCERNS** → surface the specific ADRs or recommendations flagged; use `Codex user-input tool` with options: `Revise the impact assessment` / `Accept with noted concerns` / `Discuss further`
...
- **ADRs marked Superseded**: "Run `$ccgs-architecture-decision [title]` to write the
  replacement ADR. Then re-run `$ccgs-propagate-design-change` to verify coverage."
- **ADRs to update in place**: List the specific fields to update in each ADR
- **If many ADRs affected**: "Run `$ccgs-architecture-review` after all ADRs are updated
