...
At runtime, execute this shell snippet if needed: `source "${CLAUDE_PROJECT_DIR:-.}/.claude/hooks/yaml-helper.sh" 2>/dev/null && resolve_config --keys workflow`
...
- **`minimal`** — the design artifact is the one-page `design/game-brief.md`
  ...
  `$ccgs-setup-engine` runs *first* in the minimal path, so a missing brief here is
...
- **Full spec**: `$ccgs-setup-engine godot 4.6` — engine and version provided
- **Engine only**: `$ccgs-setup-engine unity` — engine provided, version will be looked up
- **No args**: `$ccgs-setup-engine` — fully guided mode (engine recommendation + version)
- **Refresh**: `$ccgs-setup-engine refresh` — update reference docs (see Section 10)
- **Upgrade**: `$ccgs-setup-engine upgrade [old-version] [new-version]` — migrate to a new engine version (see Section 11)
...
- **`standard` / `full`**: read `design/gdd/game-concept.md` if it exists —
  ...
  recommendation from `$ccgs-brainstorm`.
...
- If no concept **or** brief exists, inform the user:
  > "No game concept found. Consider running `$ccgs-brainstorm` first to discover what
...
**Question 1 — Prior experience** (ask this first, always, via `Codex user-input tool`):
...
**Question 2 — Target platform** (ask this second, always, via `Codex user-input tool` — platform eliminates or heavily weights engines before any other factor):
...
5. Use `Codex user-input tool` to confirm: "Does this recommendation feel right, or would you like to explore a different engine?"
...
Use `Codex user-input tool` with concept-specific deep-dive topics. Always generate these options from the user's actual concept — do not use generic options. Always include at minimum:
...
**For Godot** — use the template matching the language chosen above. See `.agents/skills/ccgs-setup-engine/references/godot-language-config.md` (**A1**) for all three variants (GDScript, C#, Both).
...
every session. It is idempotent — re-running `$ccgs-setup-engine` for a different
...
**For Godot** — see `.agents/skills/ccgs-setup-engine/references/godot-language-config.md` (**A2**) for GDScript, C#, and Both variants.
...
> is true at any tier when `$ccgs-setup-engine` is run before the concept exists.
>
...
>   `$ccgs-settings` once the brief exists.
>
...
### Remaining Sections
- **Performance Budgets**: Use `Codex user-input tool`:
...
**For Godot** — see `.agents/skills/ccgs-setup-engine/references/godot-language-config.md` (**A3**) for the routing table matching the language chosen.
...
> `$ccgs-setup-engine <engine> <version>` and `$ccgs-setup-engine <engine>` skip Section 2
...
>      `Leave as [TO BE CONFIGURED]` — a legitimate answer here, revisited via
>      `$ccgs-settings` once the concept exists.
>
...
- `rendering` and `physics` are engine-typical defaults — tell the user they can
  be changed later via `$ccgs-settings` if the project uses a different setup.
...
Section 5 (**A3** in `.agents/skills/ccgs-setup-engine/references/godot-language-config.md` for Godot):
...
> emit one from recall — the same call made for `$ccgs-security-audit`'s save
...
> `$ccgs-setup-engine` usually runs before the Godot project has any — write
...
scripting; `.cs` files follow C# conventions per-file (see **A2** in `.agents/skills/ccgs-setup-engine/references/godot-language-config.md`).
...
> `$ccgs-dev-story` Phase 6 step 4 appends the per-engine capture flags to it
> (`.claude/docs/run-and-observe.md`). `test` and `smoke` feed the parse check
> and `$ccgs-smoke-check`; `build` is the one row you must ask for.
...
```yaml
commands:
  # TODO: confirm these for your UE version and project — adjust via $ccgs-settings
...
- **`project.yaml` exists** — read it first, then, processing the blocks in the order
  ...
  - **Block absent** — insert the whole block, appended after the last existing
    top-level block.
  - **Block already present** (a re-run of `$ccgs-setup-engine`) — Edit the existing
...
Run `$ccgs-setup-engine refresh` to populate full reference docs at any time.
```
...
`$ccgs-dev-story` writes `.gd`/`.cs`/`.cpp` under the resolved code root, and the `minimal` path
...
Add a comment saying `$ccgs-dev-story` or the developer sets it once a scene exists;
...
- **Unity** — create the project in Unity Hub at the repo root, choosing the
  template matching the 2D/3D answer from Section 2, then re-run `$ccgs-setup-engine`.
...
If invoked as `$ccgs-setup-engine upgrade [old-version] [new-version]`:
...
2. Run $ccgs-setup-engine refresh after upgrading the actual engine binary to
   verify no new deprecations were missed
3. Run $ccgs-architecture-review — the engine upgrade may invalidate ADRs that
   reference specific APIs or engine capabilities
4. If any ADRs are invalidated, run $ccgs-propagate-design-change to update
   downstream stories
```
...
```
2. Run $ccgs-brainstorm to produce your one-page design/game-brief.md (the lean-tier design artifact)
3. Run $ccgs-create-stories to turn the brief's MVP list into stories
4. Run $ccgs-dev-story — first line of game code
```
(No `$ccgs-map-systems`, `$ccgs-design-system`, `$ccgs-prototype`, or `$ccgs-sprint-plan` at
...
5. [If fresh start] Run $ccgs-brainstorm to discover your game concept
6. Create your first milestone: $ccgs-sprint-plan new
```
...
`.agents/skills/ccgs-setup-engine/references/godot-language-config.md`
