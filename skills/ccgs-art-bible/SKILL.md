---
name: ccgs-art-bible
description: "In a CCGS game project, author the Art Bible — visual identity gating asset production. Run before $ccgs-map-systems."
---
...
> "No game concept found. Run `$ccgs-brainstorm` first — the art bible is authored after the game concept is approved."
...
Use `Codex user-input tool` with two tabs:
...
Present the art-director's draft to the user. Use `Codex user-input tool`:
...
Collect both. If they conflict (e.g., art-director wants elaborate diegetic UI but ux-designer flags it would reduce combat readability), surface the conflict explicitly with both positions. Do NOT silently resolve — use `Codex user-input tool` to let the user decide.
...
- Do GDDs exist (check above)? → include $ccgs-consistency-check option

Use `Codex user-input tool` for next steps. Only include options that are genuinely next based on the state check above:

**Option pool — include only if not already done:**
- `[_] Run $ccgs-map-systems — decompose the concept into systems before writing GDDs` (skip if systems-index.md exists)
- `[_] Run $ccgs-setup-engine — configure the engine (asset standards may need revisiting after engine is set)` (skip if engine configured)
- `[_] Run $ccgs-design-system — start the first GDD` (skip if any GDDs exist)
- `[_] Run $ccgs-review-all-gdds — cross-GDD consistency check (required before Technical Setup gate)` (skip if gdd-cross-review-*.md exists)
- `[_] Run $ccgs-asset-spec — generate per-asset visual specs and AI generation prompts from approved GDDs` (include if GDDs exist)
- `[_] Run $ccgs-consistency-check — scan existing GDDs against the art bible for visual direction conflicts` (include if GDDs exist)
- `[_] Run $ccgs-create-architecture — author the master architecture document (next Technical Setup step)`
- `[_] Stop here`
...
> **Always include** `$ccgs-create-architecture` and Stop here as options — these are always valid next steps once the art bible is complete.
...
- Run `$ccgs-map-systems` to decompose the concept into game systems before authoring GDDs
- Run `$ccgs-setup-engine` if the engine is not yet configured (asset standards may need revisiting after engine selection)
- Run `$ccgs-design-system [first-system]` to start authoring per-system GDDs
- Run `$ccgs-consistency-check` once GDDs exist to validate them against the art bible's visual rules
- Run `$ccgs-create-architecture` to produce the master architecture document
