---
name: ccgs-team-qa
description: "In a CCGS game project, orchestrate the QA team through a full testing cycle — qa-lead strategy and test plan, qa-tester case writing, execution, sign-off."
---
...
**Decision Points:** At each phase transition, use `Codex user-input tool` to present
...
At runtime, execute this shell snippet if needed: `source "${CLAUDE_PROJECT_DIR:-.}/.claude/hooks/yaml-helper.sh" 2>/dev/null && resolve_config --keys review_mode,automation,team.size`
...
Directors (CD/TD/PR) still spawn at phase gates regardless of size; a non-core agent needed at `individual` routes through the nearest active core agent with an informational note. **"Phase gate" means any phase that ends in an `Codex user-input tool` decision point before the pipeline advances** — not every phase. Apply the test literally: if the phase below has no decision point, it is not a gate, and an agent restricted to "phase gates only" is not spawned for it. This active-set scoping applies throughout the pipeline below: any phase that names an agent outside the active set routes through the nearest core agent rather than spawning it.
...
> **Why this does not violate the Collaboration Protocol.** `CLAUDE.md` requires an agent to ask "May I write this to [filepath]?" before Write/Edit. A subagent spawned here writes **without** asking, and that is a deliberate, bounded exception rather than an oversight — the same call already made for `consistency-check` appending to `active.md`. The exception holds only when all three are true: (1) the path is one **you** named in the prompt, so the user approved the destination when they approved the phase; (2) it is a new artifact under `production/`, `docs/` or `tests/`, never an edit to existing source or config; (3) the phase that produced it is itself gated by an `Codex user-input tool` before the pipeline advances. Outside those three, the agent must ask. **Do not "fix" this by asking per subagent** — a prompt per agent per phase makes an orchestrator unusable, which is why the exception exists.
...
Present the qa-lead's full strategy to the user, then use `Codex user-input tool`:
...
options:
  ...
  - "Smoke check failed — fix issues and re-run $ccgs-team-qa"
  - "Cancel — resolve blockers first"
```

If smoke check **FAIL**: do not proceed to Phase 3. Surface the failures from the smoke check report and stop. The user must fix them, re-run `$ccgs-smoke-check sprint`, and then re-run `$ccgs-team-qa`.
If smoke check **UNKNOWN**: surface a warning — "No smoke check report found. Recommend running `$ccgs-smoke-check sprint` before QA. Proceeding with caution."
...
Use `Codex user-input tool` per story group (batched 3-4 at a time):
...
Batch stories into groups of 3-4 and use `Codex user-input tool` for each:
...
After each FAIL result: use `Codex user-input tool` to collect the failure description, then spawn `qa-tester` via `Agent` to write a formal bug report in `production/qa/bugs/`.
...
it from the doc — `$ccgs-story-done` checks for a retained image, not just the
write-up.

> **This is not optional bookkeeping — it is the artifact the next skill gates
> on.** `$ccgs-story-done` globs `production/qa/evidence/` for Visual/Feel and UI
> stories and reads the sign-off table; `$ccgs-story-readiness`,
> `$ccgs-test-evidence-review` and `gate-release` read the same directory. Write
> anywhere else and a story can pass a full manual QA cycle here, then be told by
> `$ccgs-story-done` that no visual evidence exists. Visual/Feel and UI gates are
...
- NOT ASSESSED: "QA did not run to completion. Produce the missing evidence — write the Logic tests, walk the manual cases, run `$ccgs-smoke-check` — then re-run `$ccgs-team-qa`. Do not advance the build on this verdict."
- APPROVED: "Build is ready for the next phase. Run `$ccgs-gate-check` to validate advancement."
- APPROVED WITH CONDITIONS: "Resolve conditions before advancing. S3/S4 bugs may be deferred to polish."
- NOT APPROVED: "Resolve S1/S2 bugs and re-run `$ccgs-team-qa` or targeted manual QA before advancing."
...
- ADR status is Proposed → do not implement; run `$ccgs-architecture-decision` first
- Scope too large → split into two stories via `$ccgs-create-stories`
