...
description: "In a CCGS game project, ready to advance between development phases? PASS/CONCERNS/NOT ASSESSED/FAIL with blockers and required artifacts. 'Can we move to production?'"
---
...
**Distinct from `$ccgs-project-stage-detect`**: That skill is diagnostic ("where are we?").
...
- **With argument**: `$ccgs-gate-check production` — validate readiness for that specific phase
- **No argument**: Auto-detect current stage using the same heuristics as
  `$ccgs-project-stage-detect`, then **confirm with the user before running**:
...
|------|-----------------|
| Concept → Systems Design | `.agents/skills/ccgs-gate-check/references/gate-systems-design.md` |
| Systems Design → Technical Setup | `.agents/skills/ccgs-gate-check/references/gate-technical-setup.md` |
| Technical Setup → Pre-Production | `.agents/skills/ccgs-gate-check/references/gate-pre-production.md` |
| Pre-Production → Production | `.agents/skills/ccgs-gate-check/references/gate-production.md` |
| Production → Polish | `.agents/skills/ccgs-gate-check/references/gate-polish.md` |
| Polish → Release | `.agents/skills/ccgs-gate-check/references/gate-release.md` |
...
> `.claude/docs/coding-standards.md` ("`$ccgs-smoke-check` is a build-health gate, not
...
> never noticed. `$ccgs-gate-check` is the right home: it already resolves the whole
...
Use `Glob('design/gdd/gdd-cross-review-*.md')` to find the `$ccgs-review-all-gdds` report.
If no file matches: at `full` mark the "cross-GDD review report exists" artifact as
**FAIL** and surface it prominently ("No `$ccgs-review-all-gdds` report found in
`design/gdd/`. Run `$ccgs-review-all-gdds` before advancing to Technical Setup."); at
...
- For performance checks: read the budgets (`performance.target_framerate`,
  ...
  `tests/performance/` or recent `$ccgs-perf-profile` output. What a breach *means*
...
- "Performance profiling data isn't available. Would you like to run `$ccgs-perf-profile`?"
...
not consulted. Run `$ccgs-gate-check --review full` or set `modes.workflow: full` for
...
### Blockers
1. **No Architecture Decision Records** — Run `$ccgs-architecture-decision` to create one
...
- A **quality check's input carries no measured data**. Section 3 compares the
  ...
  `$ccgs-perf-profile` output" — and `$ccgs-perf-profile`'s report template pre-fills
  ...
  `$ccgs-perf-profile`); this covers data that is *present and hollow*, which is the
...
- A **test check the tier requires could not be executed** — no test runner is
  ...
  (`$ccgs-test-setup`); this is what the verdict does with it.
...
> `standard`. Revisit with `$ccgs-settings modes.rigor=standard`?"

Offer it **once**, here at the gate. Route to `$ccgs-settings` — never change the
setting yourself.
...
After the verdict is presented and any stage update is complete (project.yaml + stage.txt), close with a structured next-step prompt using `Codex user-input tool`.
...
[A] Run $ccgs-create-architecture — produce your master architecture blueprint and ADR work plan (recommended next step)
[B] Design more GDDs first — return here when all MVP systems are complete
[C] Stop here for this session
```

> **Note for systems-design PASS**: `$ccgs-create-architecture` is the required next step before writing any ADRs. It produces the master architecture document and a prioritized list of ADRs to write. Running `$ccgs-architecture-decision` without this step means writing ADRs without a blueprint — skip it at your own risk.

For **technical-setup PASS**:
```
Gate passed. What would you like to do next?
[A] Run $ccgs-create-control-manifest — generate the layer rules manifest from your Accepted ADRs (do this first)
[B] Run $ccgs-vertical-slice — build the Vertical Slice (do this before writing epics — validate fun first)
[C] Write more ADRs first — run $ccgs-architecture-decision [next-system]
...
>
> 1. `$ccgs-create-control-manifest` — extract technical rules from Accepted ADRs (required before epics)
> 2. `$ccgs-vertical-slice` — build the Vertical Slice **FIRST**, before writing epics or stories
> 3. Playtest → `$ccgs-playtest-report` — at least 1 session required to pass the Pre-Production gate; 3+ recommended before committing the full team
> 4. `$ccgs-ux-design [screen]` — UX specs for main menu, core HUD, pause menu (if not done)
> 5. `$ccgs-create-epics layer:foundation` then `$ccgs-create-epics layer:core` — plan after fun is validated
> 6. `$ccgs-create-stories [epic-slug]` for each epic
> 7. `$ccgs-sprint-plan new`
>
...
- **No art bible?** → `$ccgs-art-bible` to create the visual identity specification
- **Art bible exists but no asset specs?** → `$ccgs-asset-spec system:[name]` to generate per-asset visual specs and generation prompts from approved GDDs
- **No game concept?** → `$ccgs-brainstorm` to create one
- **No systems index?** → `$ccgs-map-systems` to decompose the concept into systems
- **Missing design docs?** → `$ccgs-reverse-document` or delegate to `game-designer`
- **Small design change needed?** → `$ccgs-quick-design` for changes under ~4 hours (bypasses full GDD pipeline)
- **No UX specs?** → `$ccgs-ux-design [screen name]` to author specs, or `$ccgs-team-ui [feature]` for full pipeline
- **UX specs not reviewed?** → `$ccgs-ux-review [file]` or `$ccgs-ux-review all` to validate
- **No accessibility requirements doc?** → run `$ccgs-ux-design` which creates both `design/accessibility-requirements.md` and `design/ux/interaction-patterns.md` in one step
- **No interaction pattern library?** → `$ccgs-ux-design patterns` to initialize it
- **GDDs not cross-reviewed?** → `$ccgs-review-all-gdds` (run after all MVP GDDs are individually approved)
- **Cross-GDD consistency issues?** → fix flagged GDDs, then re-run `$ccgs-review-all-gdds`
- **No test framework?** → `$ccgs-test-setup` to scaffold the framework for your engine
- **No QA plan for current sprint?** → `$ccgs-qa-plan sprint` to generate one before implementation begins
- **Missing ADRs?** → `$ccgs-architecture-decision` for individual decisions
- **No master architecture doc?** → `$ccgs-create-architecture` for the full blueprint
- **ADRs missing engine compatibility sections?** → Re-run `$ccgs-architecture-decision`
  or manually add Engine Compatibility sections to existing ADRs
- **Missing control manifest?** → `$ccgs-create-control-manifest` (requires Accepted ADRs)
- **Missing epics?** → `$ccgs-create-epics layer: foundation` then `$ccgs-create-epics layer: core` (requires control manifest)
- **Missing stories for an epic?** → `$ccgs-create-stories [epic-slug]` (run after each epic is created)
- **Stories not implementation-ready?** → `$ccgs-story-readiness` to validate stories before developers pick them up
...
- **No Difficulty Curve doc?** → Author `design/difficulty-curve.md` by hand from the template at `.claude/docs/templates/difficulty-curve.md`. (`$ccgs-quick-design` is a related session but writes to `design/quick-specs/`, not to this path — use it to think the curve through, then copy the outcome here.)
...
> otherwise.** Naming "`$ccgs-ux-design` Phase 2b" would point at the step that *reads*
> `design/player-journey.md`, landing the user back at the check that just
> failed. Naming `$ccgs-quick-design` would point at a skill that writes
...
- **Ready for release?** → `$ccgs-launch-checklist`
...
6. **Never auto-fix**: If required artifacts are missing, report the FAIL verdict and
   name the skill to run (e.g. "run `$ccgs-test-setup`"). Do NOT create missing files or
