---
name: ccgs-asset-spec
description: "In a CCGS game project, per-asset visual specs plus AI generation prompts from GDDs and character profiles. After the art bible."
---
...
At runtime, execute this shell snippet if needed: `source "${CLAUDE_PROJECT_DIR:-.}/.claude/hooks/yaml-helper.sh" 2>/dev/null && resolve_config --keys review_mode,automation`
If no argument is provided, check whether `design/assets/entity-inventory.md` exists:
- If it exists: read it, find the first entity or screen with status "Needed" but no spec file yet, and use `Codex user-input tool`:
...
Present the full proposed inventory to the user in conversation. Then use `Codex user-input tool`:
...
> - Run `$ccgs-ux-design [screen name]` for each UI screen in the inventory
> - Run `$ccgs-asset-spec entity:[name]` to spec each visual entity
> - Or run `$ccgs-asset-spec` again to work through the inventory one item at a time"
...
- **Art bible**: Read `design/art/art-bible.md` — fail if missing:
  > "No art bible found. Run `$ccgs-art-bible` first — asset specs are anchored to the art bible's visual rules and asset standards."
...
- **system**: Read `design/gdd/[target-name].md`. Extract the **Visual/Audio Requirements** section. If it doesn't exist or reads `[To be designed]`:
  > "The Visual/Audio section of `design/gdd/[target-name].md` is empty. Either run `$ccgs-design-system [target-name]` to complete the GDD, or describe the visual needs manually."
  Use `Codex user-input tool`: `[A] Describe needs manually` / `[B] Stop — complete the GDD first`
...
- **character** or **entity**: Read `design/narrative/characters/[target-name].md` or search `design/narrative/` and `design/assets/entity-inventory.md` for a matching entry. Extract visual description, role, and any specified distinguishing features.
  - **If no source doc exists**: do not fail. Instead, use `Codex user-input tool`:
...
Present the full identified list to the user. Use `Codex user-input tool`:
...
After presenting all specs, use `Codex user-input tool`:
...
- Options:
  - `[A] Spec another system — $ccgs-asset-spec system:[next-system]`
  - `[B] Spec a level — $ccgs-asset-spec level:[level-name]`
  - `[C] Spec a character — $ccgs-asset-spec character:[character-name]`
  - `[D] Run $ccgs-asset-audit — validate delivered assets against specs`
  - `[E] Stop here`
...
- Run `$ccgs-asset-spec [next-context]` to continue speccing remaining systems, levels, or characters
- Run `$ccgs-asset-audit` to validate delivered assets against the written specs and identify gaps or mismatches
