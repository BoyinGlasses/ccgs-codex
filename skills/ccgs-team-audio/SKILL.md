...
description: "In a CCGS game project, orchestrate the audio team — audio-director, sound-designer, technical-artist, gameplay-programmer — direction through implementation."
---
...
> Usage: `$ccgs-team-audio [feature or area]` — specify the feature or area to design audio for (e.g., `combat`, `main menu`, `forest biome`, `boss encounter`). Do not use `Codex user-input tool` here; output the guidance directly.
...
5. **Save to** `design/audio/audio-[feature].md` — **but ask first.** `design/` is
   ...
   the document yourself and ask directly via `Codex user-input tool` — "May I write the
...
- Use `$ccgs-dev-story` to implement the audio manager and event system once the design is approved.
- Run `$ccgs-asset-audit` after audio assets are created to verify naming and format compliance.
