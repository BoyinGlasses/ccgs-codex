---
name: ccgs-create-control-manifest
description: "In a CCGS game project, flat must-do/never-do rules sheet per system and layer, extracted from Accepted ADRs. ADRs explain why; this is actionable."
---
...
**When to run:** After `$ccgs-architecture-review` passes and ADRs are in Accepted
...
count **N**. If N is 0: "No ADRs found — run `$ccgs-architecture-decision` before
...
| **0 matches, N > 0** | **Malformed ADRs — not an empty Accepted set.** `## Status` is BLOCKING-if-missing. | "[N] ADRs found, none has a `## Status` section — acceptance cannot be determined. Run `$ccgs-architecture-decision [file] retrofit` on each." **Stop.** Do not treat all ADRs as Accepted; do not emit a manifest. |
...
- Deprecated APIs → Forbidden API entries, **filtered for relevance to this
  ...
  > of rules that do not apply is one nobody reads, and `$ccgs-create-stories` consumes
  > this file.
...
- **CONCERNS** → surface via `Codex user-input tool` with options: `Revise flagged rules` / `Accept and proceed` / `Discuss further`
...
> **Status**: [Active — regenerate with `$ccgs-create-control-manifest update` when ADRs change]

`Manifest Version` is the date this manifest was generated. Story files embed
this date when created. `$ccgs-story-readiness` compares a story's embedded version
...
- If epics/stories don't exist yet: "Run `$ccgs-create-epics layer: foundation` then `$ccgs-create-stories [epic-slug]` — programmers
