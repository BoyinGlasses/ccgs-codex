...
description: "In a CCGS game project, day-one launch patch — focused fix for known issues found after gold master. Mini-sprint with QA gate and rollback."
---
...
- **No file in `production/gate-checks/`**, or none recording a release-gate
  ...
  by a gate nobody can find, and ask whether to run `$ccgs-gate-check` first or
...
If [C]: output "No day-one patch required. Proceed to `$ccgs-launch-checklist`." Stop.
...
Use `Codex user-input tool` to confirm proceeding or reduce scope.
...
This is a lightweight QA pass — not a full `$ccgs-team-qa`. The patch is already QA-approved from the release gate; we are only re-verifying the changed areas.
...
- **Targeted smoke check** — run `$ccgs-smoke-check [affected-systems]`
...
**If the QA verdict is `NOT ASSESSED`, that is not a pass.** `$ccgs-smoke-check` returns
...
1. Run `$ccgs-patch-notes` to generate the player-facing version of the patch notes
2. Run `$ccgs-bug-report verify [BUG-ID]` for each fixed bug after the patch is live
3. Run `$ccgs-bug-report close [BUG-ID]` for each verified fix
4. Schedule a post-launch review 48–72 hours after launch using `$ccgs-retrospective launch`
...
- Options:
  - `[A] Run $ccgs-patch-notes — generate player-facing patch notes`
  - `[B] Run $ccgs-bug-report to log any issues found post-deploy`
  - `[C] Stop here`
...
- **Player communication is part of the patch** — `$ccgs-patch-notes` is a required output, not optional
