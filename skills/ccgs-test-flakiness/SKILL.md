---
name: ccgs-test-flakiness
description: "In a CCGS game project, find flaky tests from CI logs — aggregates pass rates, spots intermittent failures, recommends quarantine. After multiple runs."
---
...
- After `$ccgs-regression-suite` identifies quarantined tests that need diagnosis
...
**Modes:**
- `$ccgs-test-flakiness [ci-log-path]` — analyse a specific CI run log file
- `$ccgs-test-flakiness scan` — scan all available CI logs in `.github/` or
  standard log output directories
- `$ccgs-test-flakiness registry` — read existing regression-suite.md quarantine
...
> 3. Run `$ccgs-test-flakiness registry` to review tests already flagged as flaky
