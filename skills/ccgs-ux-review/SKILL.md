---
name: ccgs-ux-review
description: "In a CCGS game project, validate a UX spec, HUD design or pattern library — accessibility, GDD alignment, readiness. APPROVED / NOT ASSESSED / NEEDS REVISION / MAJOR REVISION NEEDED."
---
...
the `$ccgs-team-ui` pipeline.

**Run this skill:**
- After completing a UX spec with `$ccgs-ux-design`
...
- **Specific file path** (e.g., `$ccgs-ux-review design/ux/inventory.md`): validate
  that one document
...
> `$ccgs-team-ui` Phase 4 and is mirrored here so the two cannot drift. If the spec's own
> header states a tier, carry it forward as an **assumption** and say plainly that
> it was assumed rather than committed. Recommend `$ccgs-ux-design accessibility` to
> establish the tier.
...
[For APPROVED]: This spec is ready for handoff to `$ccgs-team-ui` Phase 2
(Visual Design).
...
Handoff to `$ccgs-team-ui` is not recommended on this result.
...
Recommend returning to `$ccgs-ux-design` to rework [sections].
```
...
- For **APPROVED**: suggest running `$ccgs-team-ui` to begin implementation coordination
- For **NOT ASSESSED**: name the missing input per dimension and offer to help
  produce it (`$ccgs-ux-design accessibility` for an uncommitted tier, the GDD path
...
- For **NEEDS REVISION**: offer to help fix specific gaps ("Would you like me to
  ...
  instruction
- For **MAJOR REVISION NEEDED**: suggest returning to `$ccgs-ux-design` with the
