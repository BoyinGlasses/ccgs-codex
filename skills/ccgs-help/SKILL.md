---
name: ccgs-help
description: "In a CCGS game project, what should I do next? Use when stuck or you don't know what to do."
---
...
gap analysis, use `$ccgs-project-stage-detect`.
...
- If all required steps in the current phase are complete (or nearly complete),
  add: "You're close to the **[Current] → [Next]** gate. Run `$ccgs-gate-check` when ready."
...
---
Need more detail?
- `$ccgs-project-stage-detect` — full gap analysis with all missing artifacts listed
- `$ccgs-gate-check` — formal readiness check for your next phase
- `$ccgs-start` — re-orient from scratch
- `$ccgs-settings` — if the process feels mismatched to your project, adjust
...
`$ccgs-settings` rigor line only when a `settings-guidance.md § 4` trigger actually
