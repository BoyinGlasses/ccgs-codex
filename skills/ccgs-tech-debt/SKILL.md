---
name: ccgs-tech-debt
description: "In a CCGS game project, track, categorize and prioritize technical debt across the codebase — scans for debt indicators, maintains a register."
---
...
Then use `Codex user-input tool` to collect the **category**:
...
Then use `Codex user-input tool` to collect the **estimated fix effort**:
...
- Run `$ccgs-sprint-plan` to schedule high-priority debt items into the next sprint.
- Run `$ccgs-tech-debt report` at the start of each sprint to track debt trends over time.
