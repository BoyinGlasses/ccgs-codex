---
name: ccgs-adopt
description: "In a CCGS game project, brownfield audit — do existing artifacts actually work? Numbered migration plan. Unlike $ccgs-project-stage-detect, checks compliance not existence."
---
...
At runtime, execute this shell snippet if needed: `source "${CLAUDE_PROJECT_DIR:-.}/.claude/hooks/yaml-helper.sh" 2>/dev/null && resolve_config --keys automation,automation_always_ask,workflow`
Resolved above — use as-is. `$ccgs-adopt` also inspects `project.yaml` and the legacy
...
**This is not `$ccgs-project-stage-detect`.**
`$ccgs-project-stage-detect` answers: *what exists?*
`$ccgs-adopt` answers: *will what exists actually work with the template's skills?*
...
`project.yaml` → default `collaborative`). Every `Codex user-input tool` call follows `.claude/docs/automation-modes.md`
...
Use the same heuristic as `$ccgs-project-stage-detect`:
...
- Nothing → Fresh (not a brownfield project — suggest `$ccgs-start`)

If the project appears fresh (no artifacts at all), use `Codex user-input tool`:
- "This looks like a fresh project — no existing artifacts found. `$ccgs-adopt` is for
  projects with work to migrate. What would you like to do?"
  - "Run `$ccgs-start` — begin guided first-time onboarding"
...
> and `$ccgs-design-system` produce `> **Status**: …` — so an anchor of
...
|---|---|
| `## Status` | **BLOCKING** — `$ccgs-story-readiness` ADR status check silently passes everything |
| `## ADR Dependencies` | HIGH — dependency ordering in `$ccgs-architecture-review` breaks |
...
1. **Parenthetical status values** — Grep for any Status cell containing
   ...
   These break exact-string matching in `$ccgs-gate-check`, `$ccgs-create-stories`,
   and `$ccgs-architecture-review`. **BLOCKING.**
...
conflict: `$ccgs-start` writes `production/stage.txt` on every new v1.1 project as a
...
`$ccgs-architecture-decision retrofit docs/architecture/adr-[NNNN]-[slug].md`
...
`$ccgs-design-system retrofit design/gdd/[filename].md`
...
2. Run `$ccgs-architecture-review` → bootstraps `tr-registry.yaml`
3. Run `$ccgs-create-control-manifest` → creates manifest with version stamp
4. Run `$ccgs-sprint-plan update` → creates `sprint-status.yaml`
5. Run `$ccgs-gate-check [phase]` → writes `project.stage` in `project.yaml` (and legacy `stage.txt`) authoritatively
...
Gap counts:
  ...
  HIGH:     [N] — unsafe to run $ccgs-create-stories or $ccgs-story-readiness
...
Re-run `$ccgs-adopt` anytime to check remaining gaps.
...
Run `$ccgs-architecture-review` — even if ADRs already exist, this run bootstraps
...
Run `$ccgs-gate-check [current-phase]`
**Time**: 5 min
...
Run `$ccgs-adopt` again after completing Step 3 to verify all blocking and high gaps
...
**If no mode is set**: Use `Codex user-input tool`:
...
- **Options**:
  ...
  - `Lean (recommended)` — Directors only at phase gate transitions ($ccgs-gate-check). Skips per-skill reviews. Balanced for solo devs and small teams.
...
and offer to handle it immediately using `Codex user-input tool`. Choose the first
branch that applies:
...
- "The most urgent fix is `systems-index.md` — [N] rows have parenthetical status
  values (e.g. `Needs Revision (see notes)`) that break $ccgs-gate-check,
  $ccgs-create-stories, and $ccgs-architecture-review right now. I can fix these in-place."
...
- "The most urgent fix is adding `## Status` to [N] ADR(s): [list filenames].
  Without it, $ccgs-story-readiness silently passes all ADR checks. Start with
...
- "The most urgent gap is missing Acceptance Criteria in [N] GDD(s):
  [list filenames]. Without them, $ccgs-create-stories can't generate stories.
...
- "No blocking gaps — this project is template-compatible. What next?"
  ...
  - "Run $ccgs-project-stage-detect for a broader health check"
  - "Done — I'll work through the plan at my own pace"

> **Adoption plan saved to `docs/adoption-plan-[date].md`.** Re-run `$ccgs-adopt` at any time to re-check remaining gaps as you complete them.
