...
description: "In a CCGS game project, scaffold the test framework and CI — tests/ directory, engine test runner, GitHub Actions workflow. Once, before the first sprint."
---
...
1. **Read engine config**:
   ...
   - If neither source yields a configured engine (project.yaml `engine.name`
     ...
     missing), stop:
     "Engine not configured. Run `$ccgs-setup-engine` first, then re-run `$ccgs-test-setup`."
...
3. **Report findings**:
   ...
   - If everything already exists AND `force` argument was not passed:
     "Test infrastructure appears to be in place. Re-run with `$ccgs-test-setup force`
...
tests/
  ...
  smoke/          # Critical path test list for $ccgs-smoke-check gate
```
...
> where every consumer reads it — `$ccgs-smoke-check`, `$ccgs-test-evidence-review`,
> `$ccgs-qa-plan`, and the evidence table in `.claude/docs/coding-standards.md`. This
...
```gdscript
# GdUnit4 test runner — invoked by CI and $ccgs-smoke-check
...
**Run via**: `$ccgs-smoke-check` (which reads this file)
...
3. Run `$ccgs-qa-plan sprint` before your first sprint to classify stories and set
   test evidence requirements
4. `$ccgs-smoke-check` before every QA hand-off

Gate note: $ccgs-gate-check Technical Setup → Pre-Production now requires:
- tests/ directory with unit/ and integration/ subdirectories
- .github/workflows/tests.yml
- At least one example test file
Run $ccgs-test-setup and write one example test before advancing.
...
- **Engine detection is non-negotiable** — if the engine is not configured,
  stop and redirect to `$ccgs-setup-engine`. Do not guess.
