...
description: "In a CCGS game project, security audit — save tampering, cheat vectors, network exploits, data exposure, input validation. Before public or multiplayer release."
---
...
- `platform.multiplayer` and `platform.online` — whether multiplayer/networking is
  ...
  > `$ccgs-setup-engine` nor `$ccgs-start` nor any other skill emits a `platform:` block,
  ...
  >
  > Set them explicitly with `$ccgs-settings platform.multiplayer=true` (they are
...
**Audited by**: security-engineer via $ccgs-security-audit
...
Run `$ccgs-security-audit` again after remediating any CRITICAL or HIGH findings.
...
After remediating findings, re-run: `$ccgs-security-audit quick` to confirm CRITICAL/HIGH items are resolved before running `$ccgs-gate-check release`.

If CRITICAL findings exist:
> "⛔ CRITICAL security findings must be resolved before any public release. Do not proceed to `$ccgs-launch-checklist` until these are addressed."

If no CRITICAL/HIGH findings:
> "✅ No blocking security findings. Report written to `production/security/`. Include this path when running `$ccgs-gate-check release`."
