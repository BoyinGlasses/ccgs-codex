---
name: ccgs-hotfix
description: "In a CCGS game project, emergency fix bypassing normal sprint process — hotfix branch, approvals tracked, backport verified, full audit trail."
---
...
Confirm with `Codex user-input tool`:
...
If the check passes, use `Codex user-input tool` before creating the branch:
...
not as met.** `$ccgs-smoke-check` returns it when the suite never executed and
`$ccgs-team-qa` when a cycle produced no executed evidence. **A NOT ASSESSED result is
...
> tag, or deploy, use `Codex user-input tool`:
>
