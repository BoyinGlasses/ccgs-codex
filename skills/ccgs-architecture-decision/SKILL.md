---
name: ccgs-architecture-decision
description: "In a CCGS game project, create an ADR documenting a technical decision: context, alternatives considered, consequences."
---
...
At runtime, execute this shell snippet if needed: `source "${CLAUDE_PROJECT_DIR:-.}/.claude/hooks/yaml-helper.sh" 2>/dev/null && resolve_config --keys review_mode,automation,workflow,docs.density,team.size`
...
(e.g., `$ccgs-architecture-decision retrofit docs/architecture/adr-0001-event-system.md`):

Enter **retrofit mode**:

1. Read the existing ADR file completely.
2. Identify which template sections are present by scanning headings:
   - `## Status` — **BLOCKING if missing**: `$ccgs-story-readiness` cannot check ADR acceptance
...
5. If yes:
   ...
   - Append each missing section to the ADR file.
...
7. Suggest: "Run `$ccgs-architecture-review` to re-validate coverage now that this ADR
   has its Status and Dependencies fields."

**If the argument starts with `accept` followed by an ADR id**
(e.g., `$ccgs-architecture-decision accept ADR-0005`):
...
(Step 5), while
`$ccgs-create-control-manifest`, `$ccgs-create-epics`, `$ccgs-create-stories` and `$ccgs-gate-check`
...
3. **Check its dependencies first.** Read `## ADR Dependencies`.
   ...
   **If that section is absent, empty, or reads `UNKNOWN`, do not read it as
   ...
   > `$ccgs-architecture-decision retrofit <path>` to establish it."
   ...
   This is the same dependency rule `$ccgs-architecture-review` already flags; here it
...
4. **Confirm with the user, always.** Per `CONTRACT.md`, acceptance authority is
   **the user, or `technical-director` on the user's explicit confirmation — no
   other agent, and never this skill on its own.** Use `Codex user-input tool`:
...
5. **Find the stories this will unblock, BEFORE the prompt in step 4.** Grep
   ...
   > path outside `production/epics/`. `$ccgs-dev-story` matches entries *by file
...
6. **Display a knowledge gap warning** before proceeding if the domain carries
   ...
   Run `$ccgs-setup-engine` first, or tell me which engine you are using."
...
Read GDDs the same way `$ccgs-design-system` Step 2b does: only GDDs the registry
...
a **confirm/adjust** prompt using `Codex user-input tool` — not open-ended questions.
...
- **Dependencies**: Scan existing ADRs for upstream dependencies. **If the scan is
  ...
  assertion from a guess. This field is load-bearing — `$ccgs-architecture-review`
  flags unaccepted dependencies, `$ccgs-dev-story` reads it, and the acceptance route
...
decisions remain, present each one as a separate `Codex user-input tool` with the proposed
...
`$ccgs-create-control-manifest` or `$ccgs-dev-story` run should follow. State mandates as
...
5. **Write approval** — Use `Codex user-input tool`:
...
Ask using `Codex user-input tool`:
...
After the ADR is written (and registry optionally updated), close with `Codex user-input tool`.
...
[N] Start writing GDDs — run `$ccgs-design-system [first-undesigned-system]` (only show if all prerequisite ADRs are written)
[N+1] Stop here for this session
```

If there are no remaining priority ADRs and no undesigned GDD systems, offer only "Stop here" and suggest running `$ccgs-architecture-review` in a fresh session.
...
> and run `$ccgs-architecture-review`.
>
> **Never run `$ccgs-architecture-review` in the same session as `$ccgs-architecture-decision`.**
...
> Run `$ccgs-architecture-decision accept ADR-NNNN` when the decision is settled —
