...
description: "In a CCGS game project, decompose a concept into individual systems, map dependencies, prioritize design order, create the systems index."
---
...
At runtime, execute this shell snippet if needed: `source "${CLAUDE_PROJECT_DIR:-.}/.claude/hooks/yaml-helper.sh" 2>/dev/null && resolve_config --keys review_mode,automation,workflow,docs.density`
...
- **No argument**: `$ccgs-map-systems` — Run the full decomposition workflow (Phases 1-5)
  to create or update the systems index.
- **`next`**: `$ccgs-map-systems next` — Pick the highest-priority undesigned system
  from the index and hand off to `$ccgs-design-system` (Phase 6).
...
> (`$ccgs-design-system` §6 and §7, `$ccgs-create-epics`) are written against the columns it
...
- `full` / `standard` — required before `$ccgs-design-system` (systems must be mapped
...
- Read `design/gdd/game-concept.md` (at `minimal`, read `design/game-brief.md`
  ...
  > "No game concept found. Run `$ccgs-brainstorm` first to create one, then come back
...
- Use `Codex user-input tool` to ask:
...
Then use `Codex user-input tool` to capture feedback:
...
> `automation-modes.md:56` defines `autonomous` as *"No `Codex user-input tool`"*, so a
...
Use `Codex user-input tool` to ask: "Does this dependency ordering look right? Any
...
Use `Codex user-input tool` to ask: "Do these priority assignments match your vision?
...
## Phase 6: Design Individual Systems (Handoff to $ccgs-design-system)

This phase is entered when:
- The user says "yes" to designing systems after creating the index
- The user invokes `$ccgs-map-systems [system-name]`
- The user invokes `$ccgs-map-systems next`
...
Use `Codex user-input tool` for: "Start designing [system-name] now, pick a different
system, or stop here?"

### Step 6b: Hand Off to $ccgs-design-system

Once a system is selected, invoke the `$ccgs-design-system [system-name]` skill.

The `$ccgs-design-system` skill handles the full GDD authoring process:
...
- Runs `$ccgs-design-review` when complete
- Updates the systems index

**Do not duplicate the $ccgs-design-system workflow here.** This skill owns the systems
*index*; `$ccgs-design-system` owns individual system *GDDs*.

### Step 6c: Loop or Stop

After `$ccgs-design-system` completes, use `Codex user-input tool`:
...
After the systems index is created (or after designing some systems), present next actions using `Codex user-input tool`:

- "Systems index is written. What would you like to do next?"
  - [A] Start designing GDDs — run `$ccgs-design-system [first-system-in-order]`
  - [B] Run `$ccgs-gate-check systems-design` — triggers the CD-SYSTEMS and TD-SYSTEM-BOUNDARY gates automatically for a formal director sign-off on the system set
  - [C] Stop here for this session

**The gate-check option ([B]) is worth highlighting**: running `$ccgs-gate-check systems-design` triggers both the CD-SYSTEMS and TD-SYSTEM-BOUNDARY gates, catching scope issues, missing systems, and boundary problems before they're locked in across many documents. It is optional but recommended for new projects.

After any individual GDD is completed:
- "Run `$ccgs-design-review design/gdd/[system].md` in a fresh session to validate quality"
- "Run `$ccgs-gate-check systems-design` when all MVP GDDs are complete"
...
2. **Codex user-input tool** at every decision point (Explain -> Capture pattern):
   ...
   - Phase 6: "Start designing, pick different, or stop?" then hand off to `$ccgs-design-system`
...
5. **Handoff**: Individual GDD authoring is owned by `$ccgs-design-system`, which handles
...
> designing individual GDDs — run `$ccgs-map-systems next` to pick up where you left off.

---

## Recommended Next Steps

- Run `$ccgs-design-system [first-system-in-order]` to author the first GDD (use design order from the index)
- Run `$ccgs-map-systems next` to always pick the highest-priority undesigned system automatically
- Run `$ccgs-design-review design/gdd/[system].md` in a fresh session after each GDD is authored
