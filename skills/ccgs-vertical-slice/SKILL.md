---
name: ccgs-vertical-slice
description: "In a CCGS game project, pre-production validation — end-to-end build to confirm the full loop is achievable before committing to Production. After GDDs, architecture, UX specs."
---
...
whether the core idea is worth designing, run `$ccgs-prototype` (concept prototype) instead.
...
Recommended next steps:
- `$ccgs-create-epics layer:foundation` — plan Foundation layer epics
- `$ccgs-create-epics layer:core` — plan Core layer epics
- `$ccgs-create-stories [epic-slug]` — break each epic into implementable stories
- `$ccgs-sprint-plan` — plan the first sprint using velocity data from the slice
- `$ccgs-gate-check pre-production` — formally advance the stage to Production

**Playtest note:** `$ccgs-gate-check` will look for documented playtest evidence.
...
differently. When `$ccgs-vertical-slice` is next run after a PIVOT, check the
`prototypes/` directory for a `PIVOT-NOTE.md` — use it to frame the new validation
question and inform scope decisions.

- Revise affected GDDs with `$ccgs-design-system [mechanic]`
- Address architecture issues via `$ccgs-architecture-decision`
- Then re-run `$ccgs-vertical-slice` to validate the revised direction
...
- Return to `$ccgs-brainstorm` with what you learned
- Or run `$ccgs-prototype [new-concept]` to test a new direction cheaply first
