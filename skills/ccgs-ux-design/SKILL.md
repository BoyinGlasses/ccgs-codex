---
name: ccgs-ux-design
description: "In a CCGS game project, section-by-section UX spec authoring for a screen, flow or HUD. Reads the player journey to provide context; also project-wide accessibility."
---
...
At runtime, execute this shell snippet if needed: `source "${CLAUDE_PROJECT_DIR:-.}/.claude/hooks/yaml-helper.sh" 2>/dev/null && resolve_config --keys automation,workflow,docs.density`
...
> Polish gates, and `$ccgs-architecture-review` all check
...
**If no argument is provided**, do not fail — ask instead. Use `Codex user-input tool`:
...
- **Game concept**: Read `design/gdd/game-concept.md` — if missing, warn:
  > "No game concept found. Run `$ccgs-brainstorm` first to establish the game's
...
> **Do not tell the user to "run `$ccgs-ux-design` Phase 2b" to create it.** Phase 2b
...
>
> (Run `$ccgs-setup-engine` to save this permanently so you won't be asked again.)
...
2. **Questions**: Ask what is needed to draft this section. Use `Codex user-input tool`
...
3. **Options**: Where design choices exist, present 2-4 approaches with pros/cons.
   Explain reasoning in conversation, then use `Codex user-input tool` to capture the decision.
4. **Decision**: User picks an approach or provides custom direction.
5. **Draft**: Write the section content in conversation for review. Flag provisional
   assumptions explicitly.
6. **Approval**: Use `Codex user-input tool`:
...
7. **Write**: Use `Codex user-input tool`: "May I write the [section name] section to `[filepath]`?"
...
| Mode | Guidance file |
|------|---------------|
| UX Spec (screen or flow) | `.agents/skills/ccgs-ux-design/references/sections-ux-spec.md` |
| HUD Design | `.agents/skills/ccgs-ux-design/references/sections-hud.md` |
| Interaction Pattern Library | `.agents/skills/ccgs-ux-design/references/sections-patterns.md` |
| Accessibility Requirements | `.claude/docs/templates/guidance/accessibility-requirements-guide.md` |
...
> `$ccgs-ux-review` and the gate files too. Same rule applies: load only the part
...
> "This spec should be validated with `$ccgs-ux-review` before it enters the
...
- "Run `$ccgs-ux-review [filename]` now, or do something else first?"
  - Options:
    - "Run `$ccgs-ux-review` now — validate this spec"
...
`$ccgs-ux-review` on all completed specs before running `$ccgs-gate-check pre-production`."
...
- Run `$ccgs-ux-review [filename]` to validate this spec before it enters the implementation pipeline
- Run `$ccgs-ux-design [next-screen]` to continue designing remaining screens or flows
- Run `$ccgs-gate-check pre-production` once all key screens have approved UX specs
